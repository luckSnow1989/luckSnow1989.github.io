...
![](./img/9.软件工程/102192861577000.png)
...
要解决的核心问题：长任务中 AI 会“越干越废”，单 Agent 会遇到两个致命问题：
1. **上下文焦虑 / 上下文腐烂**：对话太长，上下文被填满，模型忘掉早期需求，越写越偏，甚至提前“糊弄收尾”。
2. **自我评估失灵**：让同一个 AI 写完再自审，它几乎总是给自己打高分，很难真正纠错。
   - 本质原因：**单 Agent 既当运动员又当裁判，还把所有历史都塞在同一个窗口里。**

### 1.常见方案

1. Ralph 方案：用 while 循环 + 文件系统，让 Claude Code 不断“重启会话但接干”的轻量方案。
2. 多智能体方案（推荐）：主 Agent 只协调不干活，子 Agent 各司其职，开发测试分工明确。

核心要点：**长任务别让一个 AI 又干又审又记，用“主 Agent 只调度 + 子 Agent 各司其职 + 经验库沉淀”这套多智能体 Harness，才能真正让 AI 连续干几小时还不拉胯。**

### 2.方案对比

| 维度         | Ralph 方案                                 | 多智能体方案（推荐）                    |
|------------|------------------------------------------|-------------------------------|
| 核心机制       | `while` 循环不断启动新会话，每次干净上下文                | 主 Agent 只协调，子 Agent 各司其职      |
| 上下文衔接      | 通过文件系统读写状态（如 `prd.json`, `progress.txt`） | 智能体间通过文件路径 + 标准化协议通信          |
| 工作模式       | 单一会话循环重启                                 | 开发 / 测试 / 验收等角色分工明确           |
| 适用任务       | 相对简单的线性任务                                | 复杂、需要多步骤协作的高品质任务              |
| 质量控制       | 依赖同一会话里的自纠错                              | “写 bug 的人修 bug、提 bug 的人验收”的闭环 |
| 灵活性 / 可扩展性 | 较低，流程写死在脚本里                              | 高，可动态调度不同职能子 Agent            |

Ralph 方案是“脚本驱动的简单循环”；多智能体方案是“团队驱动的协作系统”。
...
- 图片好用，可用于分享。[Superpowers：AI编程框架](https://mp.weixin.qq.com/s/o1hcLPmi6Pk__9gKqDHyFQ)
