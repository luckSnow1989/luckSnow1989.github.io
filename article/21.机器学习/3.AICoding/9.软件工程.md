...
1. 流程辅助
   - Superpowers: 全流程约束，TDD 风格，防止 Agent 乱改代码、跳过测试。开源地址：https://github.com/obra/superpowers
   - openSpec: 提供代码规范、测试用例等，确保代码质量。侧重于先定方案，再执行。（中间大模型会多次跟用户沟通设计细节
   - GSD: 长任务上下文隔离，解决“上下文腐烂”。问题。开源地址：https://github.com/gsd-build/get-shit-done
2. 多角色Agent
...
| 元技能  | `writing-skills`                 | 创建自定义技能 | 指导编写 SKILL.md   |
| 设计规划 | `brainstorming`                  | 新功能前    | 9 步需求澄清与方案设计    |
| 设计规划 | `writing-plans`                  | 设计确认后   | 拆解为 2–5 分钟细粒度任务 |
| 开发执行 | `using-git-worktrees`            | 功能开发    | 创建隔离环境          |
| 开发执行 | `test-driven-development`        | 实现功能    | 强制 TDD 循环       |
| 开发执行 | `subagent-driven-development`    | 执行计划    | 子代理独立运行         |
...
| 收尾   | `finishing-a-development-branch` | 任务完成    | 验证、合并、清理        |
