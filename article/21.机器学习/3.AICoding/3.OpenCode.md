...
- 官方插件：[https://opencode.ai/docs/zh-cn/ecosystem/](https://opencode.ai/docs/zh-cn/ecosystem/)
- 第三方插件：[https://www.opencode.cafe/](https://www.opencode.cafe/)
- 开源插件：[https://github.com/awesome-opencode/awesome-opencode](https://github.com/awesome-opencode/awesome-opencode)
...
重点：
1. OpenWork官网：[https://openworklabs.com/](https://openworklabs.com/)
...
修改智能体的默认模型：`~/.config/opencode/oh-my-opencode.json`：
...
oh-my-openagent 是一个多模型 Agent 编排引擎，作为 OpenCode 的插件运行。它将单个 AI agent 扩展为一个专业分工、协同作战的虚拟开发团队。
核心哲学是：用对的人（模型）做对的事。
...
用户请求
    ↓
    [IntentGate] — 意图分类（研究/实现/调查/修复）
    ↓
    [Sisyphus] — 主编排器，规划+委派
    ↓
    ├─→ [Prometheus] — 战略规划（面试模式，只读）
    ├─→ [Metis/Momus] — 方案质量保障
    ├─→ [Atlas] — 计划执行（指挥者）
    │    ↓
    │    ├─→ Sisyphus-Junior（category 委派 → 写代码）
    │    ├─→ Oracle（架构咨询，只读）
    │    ├─→ Librarian（文档/OSS 搜索）
    │    └─→ Explore（快速代码搜索）
```
...
| 层次  | 组件	                                          | 职责                                   |
|-----|----------------------------------------------|--------------------------------------|
| 规划层 | Prometheus + Metis + Momus	                  | 面试式需求澄清，生成 .sisyphus/plans/*.md 计划文件 |
| 执行层 | Atlas	                                       | 读取计划，分解任务，委派给工作 agent，积累经验           |
| 工作层 | Sisyphus-Junior, Oracle, Explore, Librarian	 | 实际写代码、搜索、咨询                          |

#### 5.2.核心组件详解
   
IntentGate（意图门）:
- 每个请求进来时，Sisyphus 首先分类意图：是研究（research）、实现（implementation）、调查（investigation）、还是修复（fix）？不同意图路由到不同行为模式，避免字面理解偏差。
- 本质就是一段提交给Sisyphus的prompt。
- 排歧：有没有多种理解？有没有缺关键信息？用户设计有没有坑？
- 决定路径：直接回答 → 探索 → 规划 → 实施？

Sisyphus — 主编排器：它的核心行为：
- 不直接写代码，而是规划、分解、委派
- 优先使用并行 background agents
- 内置 Todo 强制系统（完不成不罢休）
- 自称 "you write prompts, not code"

Prometheus — 战略规划师
- 只读模式，只能写 .sisyphus/ 下的 markdown 文件
- 像个真正的工程师一样面试你，追问 scope、边界、技术方案
- 生成结构化的计划文件，待 Momus 审查

Atlas — 计划执行指挥
- 读取 Prometheus 的计划，分解成原子任务
- 按 category 委派给 Sisyphus-Junior
- 每完成一个子任务，提取 learnings 传递给后续任务（Wisdom Accumulation）
- 用 boulder.json 跟踪进度，支持跨 session 恢复
...
Category 系统（核心创新）：按意图选模型，而不是按模型名选任务：
- task(category="visual-engineering", ...)  // → 前端/UI 工作（Gemini）
- task(category="ultrabrain", ...)          // → 硬逻辑/架构（GPT-5.5 xhigh）
- task(category="deep", ...)               // → 自治研究+执行（GPT-5.5）
- task(category="quick", ...)              // → 快速任务（GPT-5.4 Mini）

#### 5.3.作为插件的工作机制
oh-my-openagent 是一个 OpenCode 插件，基于 @opencode-ai/plugin SDK构建：
```json
// opencode.json
{
...
插件通过 Hooks 接口注入功能：
- config hook — 修改配置
- tool hook — 注册自定义工具（task, explore, 等所有工具）
- chat.params hook — 修改 LLM 参数
- command.execute.before hook — 拦截 /start-work 等命令
- experimental.session.compacting hook — 自定义会话压缩
