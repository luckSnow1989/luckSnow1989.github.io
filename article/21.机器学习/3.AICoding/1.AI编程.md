...
免费的工具
1. Agent-Reach。基于浏览器直接进行搜索。 https://github.com/Panniantong/Agent-Reach


### 3.MCP 服务器

大模型自带 Tool 能力，就像是电脑知道“我可以连接外部设备”；而 MCP 就像是 USB-C 接口协议。
没有 USB-C 之前，每个外设都要配专用的驱动和接口；有了统一协议后，即插即用成为可能。

传统 Tool Calling有以下几个问题
1. 每种模型的api结构都不一样，都需要各自定义不同的调用方式。尤其是GPT/anthropic/gemini等。国产模型还好。
2. 动态发现与即插即用。tool通常都是内置的代码实现的，很难进行动态变化。所以需要统一的格式实现工具调用

MCP是大模型应用的扩展，能大模型能够主动的探索外部资源

推荐使用的MCP包括：
1. Playwright MCP — 自动操作浏览器：让AI帮你打开网页、点按钮、填表单、截图。自动发帖、批量截图、抢票思路都靠它。
    - claude code、codex等采用插件的方式安装。trea等工具只能通过skills的方式安装（自动安装Playwright依赖）。
...
skills是最初是解决提示词重复编写，难以复用的难题。后面发展成调用工具外部工具、扩展SOP等。
...
见：claude code 中的 Dynamic Workflows
...
AI开发集成环境通常具备的code补全功能。依赖本地代码库生产的索引。
...
### 3.hook
钩子，用于在agent执行过程中，拦截并执行某些操作，例如日志记录、性能监控、权限控制等。
...
sandbox，为了保护本地电脑的环境，生成一个隔离的环境，防止大模型执行的代码影响到本地电脑的环境，常见的沙箱环境使用虚拟机或者docker等方式实现。
