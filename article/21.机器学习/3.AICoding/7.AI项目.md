...
- Hermes Agent：自进化型 AI 智能体，定位是任务执行引擎，主打自主学习、自我优化，像会成长的智能学徒。

二、关键能力差异
1. 技能生成逻辑
   - OpenClaw：人工投喂 / 预定义，能力全靠插件、手动配置，不主动生成新技能。
   - Hermes Agent：运行时自动生成，完成任务后自动复盘、提炼技能并持久化，越用越强。
2. 记忆与成长
   - OpenClaw：无持久自进化记忆，用完即止，能力不随使用时长提升。
   - Hermes Agent：跨会话持久记忆，持续沉淀用户习惯与任务经验，自适应适配用户 workflow。
3. 架构侧重
   - OpenClaw：网关优先，中心化调度，强稳定性、确定性，适合标准化流程。
   - Hermes Agent：学习优先，自主运行闭环，适配复杂 / 未知任务，弱化人工配置。

三、适用场景
- OpenClaw：轻量自动化、代码生成、多聊天平台整合，新手友好、个人 / 小型场景易用。
- Hermes Agent：通用复杂任务、长期自适应工作流，需要 AI 自主进化的场景更适配。
