...
nginx最大的特点是：资源消耗低，性能强、成本低。扩展性其实不算是特点，应用Apache的扩展性更强。
...
4. Apache最大的缺点是：采用fork的方式派生子进程，一个连接对应一个进程；nginx采用epoll模型，多个连接（万级别）可以对应一个进程，资源消费非常小。
5. 实际对比：Apache支撑这日均千万 PV 的网站，服务器平均负载在50~60, CPU 消耗在70%~90%；使用 Nginx 后，系统平均负载降低至1~4, CPU 使用率在20%~40%，效果可见。
...
以常见 LNMP (linux、Nginx、Mysql、PHP）为例，
- 背景：服务器在3万并发连接下，PHP部署在nginx上，且nginx开启cgi进程
...
upstream my_demo{
    ...
    # 【主动检查】2.nginx_upstream_check_module配置，如默认的基于端口存活的健康检查不满足要求，可配置基于URI的健康检查，根据返回code判断后端Server是否存活
...
2. TCP最大连接数【最大并发数】      修改 /etc/security/limits.conf 和 sysctl.conf
    # limits.conf
    * soft nofile 65535
    * hard nofile 65535
    
    # sysctl.conf
    net.core.somaxconn = 65535                  # 系统最大连接数。默认1024
    net.ipv4.tcp_max_syn_backlog = 4096         # 队列大小
    net.ipv4.ip_local_port_range = 1024 65000   # 端口范围。tcp堆外提供连接时使用。
3. 不做TCP洪水抵御                
...
1、工作进程与连接数配置
    ...
    2. worker_processes   auto          子进程数量，保持与CPU核数一致
    3. worker_connections 65535         指定单个工作进程的最大并发连接数，所有工作进程的连接数之和需小于系统的最大句柄打开数ulimit -n
    ...
    4. worker_priority -10              工作进程优先级（-20到20，数值越小优先级越高）;
    5. worker_cpu_affinity auto         CPU亲和性绑定，避免进程在CPU间切换; 
2、事件驱动模型优化
events {
    1. use epoll                        强制使用epool（Linux最优选择）
    2. multi_accept on                  允许同时接受多个连接，默认off。
    3. worker_connections  65535        连接数配置
    4. accept_mutex off                 接受连接后立即禁用accpet锁    
}

3、HTTP核心配置优化
http {
    # 1.开启零拷贝函数。sendfile_max_chunk  1m; 设置最大分片大小为1MB。
    sendfile on;    
    tcp_nopush on;    
    tcp_nodelay on;        
    
    # 2.连接超时配置(复用连接，减少创建链接的次数。前端设置，与后端的服务也需要设置。)   
    keepalive_timeout 60;            # 如果是后端服务器的nginx。禁止 keepalive_timeout，可以有效降低句柄占用数量。
    ...
    large_client_header_buffers 88k;    
    ...
    listen 443 ssl http2;        启用HTTPS并启用HTTP/2协议支持功能。
}
...
http {    
    ...
    # 2. 限制连接数：限制服务 server_name  限制客户端 binary_remote_addr
...
### 6.2.动态封禁IP
在 Nginx 中实现动态封禁 IP，可以有效保护网站或应用免受恶意请求、爬虫或 DDoS 攻击。
相比于传统的修改配置文件 + 重载 Nginx 的静态方式，动态封禁能够自动识别、实时阻断恶意 IP，大大提升安全性和运维效率。

1. Fail2ban。监控 Nginx 日志，达到阈值后调用防火墙或修改配置封禁，配置简单，社区成熟，支持多种服务。
   - 缺点：依赖日志分析，有延迟，频繁重载影响性能
   - 适用场景：防暴力破解、扫描器、异常请求攻击
2. Nginx Lua + Redis。使用 ngx_lua 在访问阶段查询 Redis 黑名单，性能高，实时生效，可分布式共享。
   - 缺点：需要 OpenResty/Lua，架构复杂
   - 适用场景：高并发场景，分布式共享黑名单，精细化策略
...
<p style="color: red">反向代理域名，域名ip变更后，nginx解析没有一起变更</p>

如果后端服务使用域名而非 IP 地址，当域名解析的 IP 发生变化时，默认情况下 Nginx 不会自动更新解析结果，需要通过以下方式处理：
1. 手动执行 reload：nginx -s reload，重启子进程。
2. 动态解析方案（无需手动 reload，适合大规模流量的情况下）

```nginx
http {
    # 指定DNS服务器（Google的公共DNS），valid=30s 表示缓存 30 秒
