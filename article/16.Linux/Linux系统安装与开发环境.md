...
![](img/linux/299c9c05b1126f3dc51b70b071dd3690.png)
...
安装: yum install -y openssl openssh-server
...
```shell
yum update
yum install tree telnet dos2unix sysstat lrzsz vim man gcc-c++ wget curl net-tools lsof sysstat
```
...
### 1.11.【日常】清理sendmail临邮件
```shell
find /var/spool/clientmqueue -type f |xargs rm -f
```

### 1.12.【可选】设置300秒不使用进入超时
```shell
export TMOUT=300
...
以下以 Ubuntu service（无界面） 16.04 LTS 为例。安装 服务版 和 桌面版有所不同，这里只介绍服务版。

[服务器版Ubuntu Server 16.04安装配置超细图解教程](https://www.cnblogs.com/jimmycan/p/5980515.html)

*【无界面版安装过程中，通常不提供WiFi驱动，所以需要连接网线到路由器，用来连接网络】*

### 2.2.创建root用户

ubuntu安装完后只有普通用户,没有ROOT用户, 因此需要手动创建ROOT帐号
...
### 2.4.连接网络
无界面版Ubuntu安装过程如果正常连接网线到路由器，则此时应该可以正常访问网络。

如果没有忘记连网线，或者安装过程选择手动配置网络的话，可以参考以下步骤。

```shell
1. 查看网卡配置： 使用 ifconfig 或 ip addr
    可以看到网卡名称，例如：ens33、eth0、enp1s3等
2. 配置网卡： sudo vim /etc/network/interfaces
    参考配置固定IP
```

连接WiFi，部分系统可能已经安装驱动。【也可以直接安装界面，方便手动设置，避免以下的繁琐设置。】
```shell
1. 安装驱动
sudo apt-get update
sudo apt-get install -y wpasupplicant wireless-tools

2. 查看系统识别的无线网卡（通常以 wlan0、wlp3s0 等命名）：
iwconfig  
或者
ip link show

3. 扫描可用 WiFi 网络： ssid是WiFi的名称
...
6. 设置DHCP client 获取IP:上面已经固定IP了，这里就不需要动态获得IP
dhclient 无线网卡名称
```

### 2.5.固定IP

1.虚拟机网络连接模式使用： 桥接模式，自动获得IP。物理机需要连接网络或者WiFi连接网络获得IP。
...
2. 修改配置：vim /etc/ssh/sshd_config
同centos

3.查看状态：sudo service ssh status
4.开启自启：sudo systemctl enable ssh
5.服务重启：sudo service ssh restart
```

### 2.7.安装界面

```shell
需要有图形界面，没有的可以执行以下代码安装图形界面（桌面版系统忽略）重启即可看到图形界面
sudo apt install ubuntu-desktop
apt-get install gnome-panel gnome-settings-daemon metacity nautilus gnome-terminal
sudo reboot 
```

界面安装之后，可能只有一个800x600的分辨率。需要修改配置
```shell
打开：/etc/default/grub
