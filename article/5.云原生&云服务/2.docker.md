...
在线中文书
- Docker — 从入门到实践:[https://github.com/yeasy/docker_practice](https://github.com/yeasy/docker_practice)
...
```dockerfile
# 1. 设置容器时区为上海。默认不带时区，使用的是子午线的时区，相差8小时。【选择合适的就行】
...
# 3. 使用非 root 用户运行服务。默认情况下，Docker 中的进程以 root 身份运行，带来安全隐患。
# 添加一个非 root 用户
RUN addgroup -S appgroup && adduser -S appuser -G appgroup
# 切换到该用户
USER appuser
# （保持 WORKDIR 在 /opt/app，不需要 chown）


# 4. 使用 .dockerignore 排除不必要文件（如 .git、pycache）
# 5. 集成 healthcheck 检测容器健康状态
# 6. 将镜像发布到 DockerHub 或 私有仓库(harbor等)
```
