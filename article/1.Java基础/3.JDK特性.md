...
JDK 17 标志着 Java 平台的重大转变，作为继 JDK 8 和 JDK 11 之后的又一个长期支持（LTS）版本，是 Java 现代化进程中的重要里程碑。
而且众多主流框架的最新版本都是支撑最低版本调整到17，也预示着以后的方向。

特性如下：
1.记录类（Record）。使用`public record Person(String name, int age) {}`就可以定义一个简单的对象类，编译器自动生成构造器、getter方法、equals、hashCode、toString方法。
- 适用场景：非常适合作为 DTO（数据传输对象）、值对象或不可变数据容器。
- 缺点：不能继承其他类，不能声明实例字段（除了在构造函数中定义的），不能是抽象的。

2.密封类（Sealed Classes）。设置本类可以被哪些类继承。`public sealed class Shape permits Circle, Rectangle, Triangle`。
- permits 关键字明确列出了允许继承该密封类的所有子类。子类必须使用 final、sealed 或 non-sealed 修饰符来声明自己的继承策略
- 密封类非常适合领域建模，特别是当你有一个封闭的类型集合时：
```java
public sealed interface PaymentMethod permits  , DebitCard, BankTransfer, DigitalWallet {
    boolean processPayment(double amount);
}

public final class   implements PaymentMethod {
    @Override
...
3.类型模式匹配。instanceof之后可以自动类型转换并赋值到变量中，简化操作。
```java
// 新方式
...
4.switch 表达式增强。
- switch可以匹配类型，自动类型转换并赋值到变量中，简化操作。
- 作为表达式使用，并且可以直接返回值，不用在写break了。
```java
...
5.文本块。与python一样，三个"内部的内容随意编写，不用拼接，随便换行不用使用换行符。SQL、JSON编写友好。

6.var增强。类型自动匹配，和golang、python一样了。
...
9.增强的 NullPointerException。打印更加详细的信息，不用再去猜哪个对象为null了。
Exception in thread "main" java.lang.NullPointerException: Cannot invoke "person.getName()" because "person" is null

10.新的垃圾收集器。包括 ZGC，它能够处理 TB 级别的堆内存，同时保持低于 10ms 的暂停时间。

11.新的堆外内存访问 API。允许 Java 程序安全地访问堆外内存
```java
// 分配堆外内存
