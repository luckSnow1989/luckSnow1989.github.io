...
当一个类加载器收到类加载请求时，不会立刻去加载这个类，而是把这个请求委派给父类加载器去完成，每一层的类加载器都是如此，直到委派到最顶层的启动类加载器。
只有当父类加载器无法完成该加载请求（即在它的搜索范围内找不到所需的类）时，子加载器才会尝试自己去加载。
...
在 Java 程序启动和运行过程中，环境变量会被 JVM、Java 工具或相关框架读取和使用。这些变量主要用于配置 JVM 行为、指定路径、设置代理等，以下是常见的环境变量及其作用：

以下是Java程序启动和运行过程中常见的环境变量整理表格：

| 环境变量名称          | 作用说明                                                                 | 示例                                   |
|-----------------------|--------------------------------------------------------------------------|----------------------------------------|
| `JAVA_HOME`           | 指定JDK/JRE的安装目录，供Java工具（如Maven、Tomcat）定位执行文件          | `JAVA_HOME=/usr/lib/jvm/openjdk-17`    |
| `CLASSPATH`           | 指定类加载器搜索`.class`文件、JAR包的路径（优先级低于`-cp`命令行参数）    | `CLASSPATH=./lib/*:/usr/share/java/utils.jar` |
| `JRE_HOME`            | 指定JRE安装目录，部分工具用于定位运行时环境（通常与`JAVA_HOME`关联）      | `JRE_HOME=/usr/lib/jvm/openjdk-17/jre` |
| `JAVA_LIBRARY_PATH`   | 指定Native库（如JNI的`.so`/`.dll`）的搜索路径                            | `JAVA_LIBRARY_PATH=/usr/local/lib:/opt/custom/native` |
| `JAVA_TOOL_OPTIONS`   | 传递JVM参数（如agent配置），被多数JVM实现支持                            | `JAVA_TOOL_OPTIONS=-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5005` |
| `_JAVA_OPTIONS`       | 传递JVM参数（兼容性略差，部分JVM支持），优先级低于命令行参数              | `_JAVA_OPTIONS=-Xmx512m -XX:+UseG1GC`  |
| `JAVA_SECURITY_POLICY`| 指定Java安全策略文件路径，控制代码访问系统资源的权限                      | `JAVA_SECURITY_POLICY=/etc/java/security/custom.policy` |
| `HTTP_PROXY`          | 配置HTTP网络代理，供Java网络请求（如URLConnection）使用                  | `HTTP_PROXY=http://proxy.example.com:8080` |
| `HTTPS_PROXY`         | 配置HTTPS网络代理，作用同`HTTP_PROXY`                                    | `HTTPS_PROXY=https://proxy.example.com:8080` |
| `NO_PROXY`            | 指定无需代理的地址（如本地地址、特定域名）                               | `NO_PROXY=localhost,127.0.0.1,.example.com` |
| `ANT_OPTS`            | 为Ant工具的JVM进程传递参数（如堆大小、日志配置）                         | `ANT_OPTS="-Xmx1024m -Dlog.level=info"` |
| `MAVEN_OPTS`          | 为Maven工具的JVM进程传递参数                                             | `MAVEN_OPTS="-Xmx2048m -XX:MetaspaceSize=256m"` |

注意：
1. 不同JVM厂商（如OpenJDK、Oracle JDK、IBM JDK）可能有专属环境变量（如IBM JDK的`IBM_JAVA_OPTIONS`），需参考具体厂商文档。
2. 命令行参数（如`-Xmx`、`-cp`）优先级通常高于环境变量。
3. 部分变量（如`JAVA_TOOL_OPTIONS`）可能被用于注入调试/监控工具，需注意安全配置。