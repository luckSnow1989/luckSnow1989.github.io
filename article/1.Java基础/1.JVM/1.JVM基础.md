...
### 2.1.介绍
- 解释器：将代码编译成虚拟机执行的字节码，可以实现语言跨平台。比如JavaScript、python等，当然Java也是。
- 编译器：将代码编译成机器执行的机器码。比如Java、golang等。

JVM中，java代码会被编译成字节码，这是一种中间形态，用于不同平台的JVM执行。
...
JIT技术，是将运行频率很高的字节码直接编译为目标机器指令（以 method 为翻译单位，还会保存起来，第二次执行就不用翻译了，直接执行）。
...
- 解释器-Interpreter：执行java字节码,HotSpot中只有一个没有具体的名字。[python解释型语言的说明](https://blog.csdn.net/puqiuzhe5231/article/details/80035316)
...
#### 2.3.1.工作模式
JVM的工作模式：解释模式、编译模式、混合模式(默认)。注意：工作模式选择的是编译器(C1 和 C2)，跟解释器无关。
...
- -Xint代表解释模式(interpreted mode)，只使用C1，在JIT编译过程中，不会对代码进行深入的性能优化，这会降低运行速度，通常低10倍或更多。
- -Xcomp代表编译模式(compiled mode)，只使用C2，在JIT编译过程中，直接对代码进行深层次的性能优化，从而带来最大程度的优化。但是也导致了更多的内存消耗和性能损耗。
- -Xmixed代表混合模式(mixed mode)，JVM的默认工作模式。C1 和 C2 混合使用，对代码进行分层编译。
...
JIT编译器与通常说的javac编译器不同，javac是将代码编译为JVM可以执行的字节码，而JIT是将字节码编译为硬件可执行的机器码。
Java中的JIT编译器，有C1 和 C2两个。
...
1. 启动类加载器  BootStrap ClassLoader:   ${JAVA_HOME}/lib 目录下的，并且是虚拟机识别的类库
2. 扩展类加载器  Extension ClassLoader:   ${JAVA_HOME}/lib/ext 目录下全部的，开发者可以直接使用,jdk9将此改名为PlatformClassLoader。
3. 应用程序加载器 Applocation ClassLoader：  加载用户classpath上指定的类库，开发者可以直接使用一般情况这个是程序默认的类加载器
...
<p style="color: red">打破双亲委派</p>

第一：SPI机制

JDK1.6之前的SPI机制。例如JNDI是JDK提供的扩展机制，是由BootStrap ClassLoader进行加载的。但是厂商扩展的驱动通常都是有appClassLoader加载的。
这就导致了JNDI的接口无法调用SPI的实现类（因为父类加载器不可能反向向子类加载findclass）。

解决方案：使用了Thread Context ClassLoader。他会通过Thread的setContext方法传递ClassLoader，
这样就可以强制使用BootClassLoader去加载第三方的类。
...
第二：热加载机制
以Tomcat为例，Tomcat自己内部构建了一套classloader机制。例如为每个webapp都单独绑定一个WebAppClassloader，
