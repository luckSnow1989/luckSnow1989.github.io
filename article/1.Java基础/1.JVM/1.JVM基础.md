...
从而带来最大程度的优化。虽然绕开了缓慢的解释器，但是很多应用在使用-Xcomp也会有一些性能损失，
但是这比使用-Xint损失的少，原因是-Xcomp没有启用JIT编译器的全部功能。所以我们并没有看到-Xcomp比-Xmixed快多少

- -Xmixed代表混合模式(mixed mode)，JVM的默认工作模式。
...
这样将频繁调用的部分提取出来，编译成本地代码，也就是在应用中构建某种热点（这也是HotSpot JVM名字的由来）。
使用混合模式可以获得最好的执行效率。
...
双亲委派模型:就是一个底层的加载器把一个class交给自己的上级，让上级去加载，上级再去找上次，如果上级不符合的话，加再交给下级去真正的加载。

![image](img/3.JVM对象模型/image11.jpeg)

1. 启动类加载器  BootStrap ClassLoader:   ${JAVA_HOME}/lib 目录下的，并且是虚拟机识别的类库
2. 扩展类加载器  Extension ClassLoader:   ${JAVA_HOME}/lib/ext 目录下全部的，开发者可以直接使用
