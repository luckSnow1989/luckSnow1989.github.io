...
- [《Java8虚拟机规范》目录](https://docs.oracle.com/javase/specs/jvms/se8/html/index.html)
...
- 堆(Heap)
- 运行时常量池（JDK6在方法区内部, JDK7被独立到堆中，JDK8合并到元空间）
...
程序运行时，内存中的信息大致分为两类，一是跟程序执行逻辑相关的指令数据，这类数据通常不大，而且生命周期短；
...
- JIT即时编译器编译后的字节码：存储的内存区域叫做 “Code Cache”，独立于方法区。
- 静态变量：静态变量区
- 常量：常量池表
