...
漏标的解决方案都是基于写屏障实现的，都需要记录，而增量更新记录变更后的引用关系，快照记录变更前的。
...
ParNew 收集器是新生代常用的垃圾收集器，是Serial的多线程版本。GC过程中全程STW。
...
先使用复制算法复制region中存活的对象，复制对象和修改对象引用需要STW，时长跟对象的数量和对象的复杂度有关，STW时长受到GC预期时间控制，该阶段通常是各阶段中最久的。

最后region中剩余的垃圾对象通过清除算法进行回收，该过程不会STW。
...
<p style="color: red">GC过程中的安全点和安全区域？</p>
...
4. CMS 不等于Full GC，我们可以看到CMS分为多个阶段，只有STW的阶段被计算到了Full GC的次数和时间，
   而和业务线程并发的GC的次数和时间则不被认为是Full GC。也就是一次CMS算是2次Full GC。

<p style="color: red">CMS垃圾收集器的并发模式失败是怎么回事？如何优化？</p>
由于年轻代增长过快，短期的对象大量进入老年代，导致老年代碎片话严重，导致无法分配大对象。出现这个问题
1. 优先增大堆内存，如果不行则适当增加年轻代的比例，避免对象大量 晋升失败。
2. 配置FullGC次数到达后进行整理一次。

<p style="color: red">高并发系统为何建议选择G1垃圾收集器？</p>
高吞吐量使用G1，低延迟使用ZGC，两者不可兼得
1. mixGC会自动进行整理，yongGC使用标记复制算法，不会产生内存碎片
2. 取消固定的年代比例，使用大小一样的region作为内存空间，GC和内存分配更加灵活
3. 可以自己单独设置STW时间。延迟会更低更稳定

<p style="color: red">解释下JVM垃圾收集底层的三色标记算法</p>
是 GC roots 标记的对象状态的一种实现。
黑：扫描过的且是存活的对象，且它所有引用链都完成扫描
灰：扫描过的且是存活的对象，引用链没有扫描完
白：没有扫描过的。如果扫描结束后还是白色的，就说明这个对象是不可达的

<p style="color: red">JVM会发生对象漏标吗？</p>
JVM为了防止漏标对象，导致对象误删。所以垃圾回收器都会在初始化标记之后，会再进行一次重新标记。
重新标记时一般有2个方案进行处理
1. 增量更新：黑色对象一旦新插入了指向白色对象的引用之后，它就变回灰色对象了。用于重新标记
2. 原始副本：就是当灰色对象在删除指向白色对象的引用关系时， 就将这个要删除的引用记录下来， 在并发扫描结束之后， 
   再将这些记录过的引用关系中的灰色对象为根， 重新扫描一次，这样就能扫描到白色的对象，将白色对象直接标记为黑色
   (目的就是让这种对象在本轮gc清理中能存活下来，待下一轮gc的时候重新扫描，这个对象也有可能是浮动垃圾)

对于读写屏障，以Java HotSpot VM为例，其并发标记时对漏标的处理方案如下：
- CMS：写屏障 + 增量更新。GC期间记录所有变更后的对象引用
- G1，Shenandoah：写屏障 + SATB。记录变化前的指针，在重新扫描阶段不扫描对象，直接标记为黑色。
- ZGC：读屏障。内部采用 转发表，记录对象的新旧地址，当程序操作对象时进行指针的变更。

<p style="color: red">为什么G1用SATB？CMS用增量更新？</p>
SATB相对增量更新效率会高(当然SATB可能造成更多的浮动垃圾)，因为不需要在重新标记阶段再次深度扫描被删除引用对象，
而CMS对增量引用的根对象会做深度扫描，G1因为很多对象都位于不同的region，CMS就一块老年代区域，
重新深度扫描对象的话G1的代价会比CMS高，所以G1选择SATB不深度扫描对象，只是简单标记，等到下一轮GC再深度扫描。


<p style="color: red">跨代引用回收问题</p>

解释这行代码的重要性：[https://mp.weixin.qq.com/s/jT_0CGok52F9autJ_N-anw](https://mp.weixin.qq.com/s/jT_0CGok52F9autJ_N-anw)

LinkedBlockingQueue#dequeue中的: h.next = h，这行的作用：

如果没有这行代码，要是head进入老年代，在youngGC的时候虽然head是垃圾了，但是head指向了next，这是跨代引用，
导致next无法被回收掉，age足够后，都进入了老年代，导致加快了fullGC。所以增加这行代码，去除跨代引用，帮助GC

h.next = h 是否可以被 h.next = null。

在帮助GC方面，作用是一样的。但是写成自循环的目的是为了帮助队列使用迭代器遍历。如下代码中可以看出，让发现p是自循环的话，
就将当前head.next返回，算是一种特殊处理。在多线程操作时，持有旧head的线程遍历队列，而其他线程出队列元素后，
这个时候就走下面的逻辑，如果设置为null的话，会导致旧线程走下面的逻辑，无法遍历全部元素。
```java
...
[从CMS到G1：LinkedIn个人主页调优实战](https://mp.weixin.qq.com/s/tbWwJ0g0AoLPMhOvemRjiw)