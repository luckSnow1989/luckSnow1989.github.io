...
新生代中分为一个 Eden 区和两个 Survivor 区。通常两个区域的比例是 8:1:1
，使用时会用到 Eden 区和其中一个 Survivor
区。当发生回收时则会将还存活的对象从 Eden ，Survivor 区拷贝到另一个
Survivor 区，当该区域内存也不足时则会使用分配担保利用老年代来存放对象。
...
- 堆
    ...
    - 新生代
        ...
        - Tenured Gen 担保区
            ...
            Survivor 有两个，Eden 一个，比例一般是
            Eden:s0:s1=8:1:1,存活的对象会在s0和
            s1之间交换位置。存活的对象需要Tenured
            进行担保才可以继续在survivor区，如果survivor区不够用了，会将对象复制到老年代中（如果一个对象非常大的话，JVM会直接在老年代中创建对象）
...
标记整理算法也可以称之为
标记-压缩算法。将垃圾对象清理掉后，同时将剩下的存活对象进行整理挪动（类似于windows的磁盘碎片整理），保证它们占用的空间连续，这样就避免了内存碎片问题，但是整理过程也会降低GC的效率，所有该算法一般作用于老年代。
...
纵向y轴可以理解分配内存的字节数，横向x轴理解为随着时间流逝（伴随着GC），可以发现大部分对象其实相当短命，很少有对象能在GC后活下来。因此诞生了分代的思想，以Hotspot为例（JDK
7）：
...
- Full
    GC：是清理整个堆空间—包括新生代、老年代、永久代和引用的堆外内存，也就是说Major
    GC是其一部分。
...
HotSpot
JVM把新生代分为了三部分：1个Eden区和2个Survivor区（分别叫from和to）。默认比例为8：1。大多数情况下，对象优先在Eden区中分配。当Eden区中没有足够空间进行分配时，将会触发一次Minor
GC。
...
Azul Zulu Prime JVM 垃圾收集器。特点如下：
1. 是不同的分代使用可以同时独立活动的并发（不会发送STW）机制进行收集。
