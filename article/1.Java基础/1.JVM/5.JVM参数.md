...
消金电催前端，新增了给图片增加水印的功能，因为每个图片都有几兆，用户一多，内存的使用率就上去了。确实cms的次数由原来的几天一次，变成不到5分钟就一次

背景：jdk8，使用CMS，堆内存共4GB，5个节点。

分析原因：因为调用次数增加，且图片数据大，数据多，所有在gc的时候，survivor放不下了，出现了新生代对象MaxTenuringThreshold
没有到达阈值就动态年龄晋升的现象，这样一来老年代的内存增长加快，major gc频率加快到5分钟一次，每次STW时间约为800ms。

解决方案：
1. 加大新生代/老年代比例为1:1，这样即使新生代对象全部进去来老年代，内存也是足够的。
2. 调整MaxTenuringThreshold=6（原本15），防止对象年龄过长一直占用新生代内存，导致再次进入动态年龄。
3. CMSInitiatingOccupancyFractionCMS= 60，触发的major gc阈值低一点（60%），高频cms与短的STW, 设置进行FullGC后0次进行整理，就是每次都整理，防止发生 promotion failed 与 concurrent mode failure (发生后CMS会降级为 serial old，导致更久的STW)

除此之外的优化手段：
1. 可以增加堆内存，选用G1。
2. 优化程序。比如：提前关闭文件里，将图片生成base64以字符串的形式发送出去，减少IO存活的时间。


效果：FULL GC 几天发生一次。因为新生代空间大了，youngGC 增加了parNew处理时间，到达10ms~25ms（原来10ms以内），
GC间隔从5秒增加到20秒，这样对象更能释放掉，对象从Eden进入survivor的数量会相对变少。minior gc时，survivor内存的不足的情况会极少发生。
