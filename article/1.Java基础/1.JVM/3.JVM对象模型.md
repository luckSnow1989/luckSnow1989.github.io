...
- 写后读：用volatile修饰的变量，当一个线程修改变量后，JVM会立即将数据同步到主内存中，并且会其他线程的工作内存中的该变量的值失效。
  当其他线程读取变量时，就会从主内存加载最新的值。
- 内存屏障：通过在volatile变量的读写操作前后插入内存屏障，可以确保变量的读写操作的指令不会被重排序
  - 写操作，在后面插入写屏障（Store Barrier），确保volatile写操作不会被重排序到屏障之后的其他写操作之前。
  - 读操作，其前面插入一个读屏障（Load Barrier），确保volatile读操作不会被重排序到屏障之前的其他读操作之后。

### 3.2.可见性

用volatile修饰的变量，线程在每次使用变量的时候，都会读取变量修改后的最新值。
但是由于操作不是原子性的，对于volatile修饰的变量，jvm虚拟机只是保证从主内存加载到线程工作内存的值是最新的。
