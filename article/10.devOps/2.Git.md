...
金融级分支开发管理模型：
![](./img/git/media/34026694731000.png)

### 1.5.分支管理规范

![](./img/2.Git/2663331512100.jpg)

一、分支命名规范
1. master（主分支）：部署生产环境，需高度稳定；仅可由release/hotfix分支合并，禁止直接修改代码。
2. develop（开发分支）：承载最新完成及bug修复后的代码，用于前后端联调；所有feature分支均基于此创建。
3. feature（功能分支）：开发新功能，命名以feature/开头（如feature/user_module）。
4. test（测试分支）：供测试人员使用，版本稳定，外部用户不可访问。
5. release（预上线分支）：供UAT测试使用，由test/hotfix分支合并，不建议直接修改。
6. hotfix（紧急修复分支）：修复线上bug，基于master创建；修复后需合并至master和develop。

二、分支与环境对应
1. 环境分为DEV（开发者调试）。develop对应DEV（可访问）
2. FAT（功能验收测试）。test对应FAT（可访问）
3. UAT（用户验收测试）。release对应UAT（可访问）
4. PRO（生产）。master对应PRO（可访问）
5. feature和hotfix无对应环境，不可访问。

三、Commit类型
1. feat（新增功能）
2. fix（修复bug）
3. docs（仅修改文档）
4. style（格式调整，不影响代码含义）
5. refactor（代码重构）
6. perf（性能优化）
7. test（补充/修正测试）
8. chore（修改构建流程/辅助工具/文档等）。

四、提交注意事项
1. 提交问题需归属同一类别；
2. 单次提交问题不超过3个；
3. 提交信息不规范时，可通过git commit --amend -m "新信息" 或 git reset --hard HEAD 重新提交。
...
## 3.客户端
- Git Bash。纯命令行客户端。
- Git GUI。官方自带的简易图形化客户端。
- 集成开发环境（IDEA,eclipse）提供的插件已足够使用。
- 小乌龟
- sourcetree
- ugit。腾讯开源的Git客户端。比较好用。[https://ugit.qq.com/zh/](https://ugit.qq.com/zh/)
...
[https://help.gitee.com/repository/ssh-key/configure-multiple-ssh-keys](https://help.gitee.com/repository/ssh-key/configure-multiple-ssh-keys)
