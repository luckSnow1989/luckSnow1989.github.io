...
### 1.5.替代产品
1. maven daemon。就是mvnd，主要解决maven编译效率低的问题。大约提升3~5倍的效率。
   - 教程：[https://mp.weixin.qq.com/s/AegtKbMLuCDy404YSgzbbw](https://mp.weixin.qq.com/s/AegtKbMLuCDy404YSgzbbw)
   - 源码：[https://github.com/apache/maven-mvnd](https://github.com/apache/maven-mvnd)
...
方式2：打开 maven 的配置文件（conf/settings.xml）
<mirror>
    <id>aliyunmaven</id>
    <name>aliyun-central</name>
    <url>https://maven.aliyun.com/repository/central</url>
...
仓库类型Maven仓库主要分为三种类型：
1. 本地仓库：位于开发者本机的仓库，默认位置为~/.m2/repository
2. 远程仓库：通过网络访问的仓库，包括：
   - 中央仓库：Maven社区提供的公共仓库
   - 私服：团队或组织内部的私有仓库
   - 第三方仓库：其他组织维护的公共仓库
...
仓库优先级与查找顺序：
1. 本地仓库：永远是第一查找目标。如果本地仓库中已存在所需版本的构件，Maven 会直接使用它（除非是 SNAPSHOT 版本且配置了更新策略）。
2. 远程仓库 (如果本地未找到):Maven 会按照一个“有效仓库列表”来查找。这个列表的构建和优先级受以下因素影响：

远程仓库优先级
1. Mirrors (settings.xml 中的 <mirrors>):镜像配置具有最高优先级，它可以覆盖 pom.xml 或 settings.xml 中 profile 定义的任何仓库。多个镜像依次查找。 
2. Profiles (settings.xml 中的<profiles>):settings.xml 中的激活的 profile 内定义的仓库。
3. pom.xml 中直接定义的仓库:<repositories>和<pluginRepositories>标签内直接定义的仓库。
4. Super POM / 中央仓库:如果以上都没有定义，或者都没有找到构件（且没有被镜像覆盖），就使用默认的中央仓库 (https://repo.maven.apache.org/maven2/)。
...
仓库配置更新策略
1. SNAPSHOT 更新: 对于 SNAPSHOT 版本的构件，Maven 会根据仓库配置中的<snapshots><updatePolicy>(如 always, daily, interval:X, never) 来决定是否检查远程仓库是否有更新版本，即使本地已存在。
2. Release 更新: 对于 Release 版本的构件，一旦下载到本地仓库，Maven 默认不会再次检查远程仓库。除非编译时使用-f 强制更新
    ...
    <profile>
      ...
      <repositories>
        <repository>
          ...
          <url>http://some.snapshot.repo/snapshots</url>
          <releases><enabled>false</enabled></releases><!-- 默认不更新 -->
          <snapshots><enabled>true</enabled></snapshots><!-- 默认更新 -->
        </repository>
      </repositories>
    </profile>
</profiles>
...
如果远程仓库需要认证(deploy jar到仓库)，需要在 settings.xml 的<servers>部分配置<server>，其<id>必须与<repository>或<mirror>的<id>匹配。

```xml
<servers>
    <server>
      <id>nexus-repo</id><!-- 与mirror的id或repository的id匹配 -->
...
按照这个规则，继承关系中的所有pom叠加到一起，就生成一个最终生效的effective pom。

查看effective pom： mvn help:effective-pom  {可选参数可以查看依赖的来源，-Dverbose}
...
maven下载依赖时，先访问镜像仓库，如果镜像仓库没有，再访问远程仓库。
maven conf目录下的settings.xml文件，可以配置mirror、profile、server等信息。
设置之后，项目本地的pom中的仓库信息就无效了，也就无需添加仓库的任何信息了，做到仓库依赖的透明。

部分公司在构建项目时会指定setting文件：mvn clean install -s my-settings.xml，帮助不通的项目使用不通的的仓库。


```xml
<servers>
      <!-- 上传maven仓库的账号密码, id 对应到镜像或者仓库的id -->
      <server>
          <id>company-repo</id>
...
<mirrors>
  ...
  <mirror>
    <id>aliyun-central</id>
    <mirrorOf>central</mirrorOf> <!-- 关联id为central的仓库 -->
    <url>https://maven.aliyun.com/repository/central</url>
    <mirrorOf>central</mirrorOf> <!-- 只有使用中央仓库的依赖才访问该镜像 -->
  </mirror>
  
  <!-- 覆盖企业私有仓库的镜像（内部镜像） -->
  <mirror>
    <id>internal-mirror</id>
    <mirrorOf>company-repo</mirrorOf> <!-- 关联id为company-repo的仓库 -->
    <url>https://mirror.company.com/maven</url> <!-- 所有的请求都会转发到该镜像 -->
    <mirrorOf>*</mirrorOf>
  </mirror>
</mirrors>
    ...
    <profile>
        ...
        <repositories>
            <repository>
                ...
                <url>https://mirror.company.com/maven</url>
...
## 7.Gradle
gradle是一个基于Java的构建工具，它基于Groovy脚本语言，提供了更灵活的构建配置和插件机制。

编译的灵活和可编程化，即是gradle最大的优势，也是它的最大缺点。

![](./img/6.maven/1123711131492600.png)


gradle对比maven

![](./img/6.maven/1123805095969599.png)
