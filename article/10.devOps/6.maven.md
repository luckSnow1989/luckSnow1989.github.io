...
Maven是Java项目管理工具，帮助开发人员自动构建、管理项目依赖关系等。Maven基于XML定义项目结构和构建过程，提供标准的构建生命周期和插件来执行各种构建任务。
...
2. Maven 自带工具。如下图，可以以图的形式展示依赖树，以及冲突
3. 使用idea插件，maven helper，与eclipse的效果一样，非常好用。

4. ![image](img/maven/media/image2.png)

### 3.3.快照与版本
快照是一种特殊的版本，指定了某个当前的开发进度的副本。
- 对于版本，如果 Maven 以前下载过指定的版本文件，比如说 1.0，Maven 将不会再从仓库下载新的可用的1.0 文件。若要下载更新的代码必须版本升到1.1。
- 对于快照，Maven 每次构建都会在远程仓库中检查新的快照，每次编译都会从仓库下载最新的仓库。
...
- maven-jar-plugin：maven：打包为一个可以执行的jar。
- maven-assembly-plugin:  支持定制化打包方式，将编译后的打包结果再处理一次。 
