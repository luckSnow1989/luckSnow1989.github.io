...
流量回放的目的：通过对线上真实流量（例如http请求）捕获、存储、重放等操作，实现非生产环境中对用户行为的模拟。

适用场景：1、回归测试（例如OceanBase数据提供的OMA迁移评估工具）；2、性能测试包括全链路压测（通过流速实现）；3、问题复现。

关键技术：
1. 流量录制：
   - 网络层实现方案：使用goReplay、tcpDump(回放使用TCPCopy)等工具实现原始数据包的抓取。
   - 应用层实现方案：通过AOP或者Java Agent技术实现对请求信息的获取。
2. 流量存储：通常为本地存储，这里可以使用脚步同步数据进行汇聚。
3. 流量回放：动态参数替换（例如token、时间戳等信息）；并发控制（提供流速控制）；协议转换（可能存储多种协议，需要进行转换）

难点：
1. 动态参数替换：1、通过正则表达式替换固定数据；2、直接覆盖原始数据，例如压测时直接生成新的token
2. https请求处理：
   - 通常使用中间人捕获流量，例如nginx获得卸载证书后的请求，再配合日志实现请求捕获。
   - 基于Linux内核探针技术实现，例如eBPF，基于内核态探针捕获 SSL/TLS 握手密钥，无需 CA 证书即可解析 HTTPS 明文
3. 数据隔离性：1、创建影子库（数据量和表结构都与生产环境一致）；	2、通过Java Agent技术实现Dao层的接口的mock。
...
源码:[https://github.com/mitmproxy/mitmproxy](https://github.com/mitmproxy/mitmproxy)

mitmproxy是一个交互式的、支持 SSL/TLS 的拦截代理，具有用于 HTTP/1、HTTP/2 和 WebSockets 的控制台界面。

mitmdump是 mitmproxy 的命令行版本。类似于 HTTP 的 tcpdump。

mitmweb是 mitmproxy 的基于 Web 的界面。