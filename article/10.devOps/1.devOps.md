...
2. 需求流程管理
   ...
   - openproject。完全开源，提供任务跟踪、甘特图、看板、时间记录、文档协作、会议管理等全栈功能，支持自托管（On-Premise）或使用官方 SaaS 服务。[https://github.com/opf/openproject](https://github.com/opf/openproject)
3. 技术设计
...
- SpringDoc。用于解决swagger的配置繁琐等问题。也是通过注解的方式简化API文档的编写。https://mp.weixin.qq.com/s/auS6DgsE26s7yaPaPzFuDA
...
3. Gerrit：Google开源代码审查工具，提供了严谨的代码审查流程，助力团队把控代码质量。

商业的很多：奇安信代码卫士、华为的CodeCheck等

#### 2.3.3.安全扫描
需要扫描的内容比较多，所以并非每次CI流程都会进行该步骤，可以设置为定期检查。
主要包括：开源漏洞检查、安全漏洞检查、开源协议检查、web漏洞检查、容器镜像检测等。

1. dependency-check，可以分析项目的安全漏洞。通过同步漏洞数据库到本地进行对比。
2. OWASP ZAP：Web应用安全扫描器，用于检测Web应用的安全漏洞，保障Web应用的安全性。
3. trivy。支持开源漏洞检查、安全漏洞检查、容器镜像检测等的检测。
...
详细见[/12.架构/4.解决方案.md#灰度发布](/12.架构/4.解决方案.md#灰度发布)
...
1. Google Analytics：网站流量数据分析工具，可对网站流量进行分析，为运营决策提供数据依据。
2. 神策数据：用户行为分析平台，能分析用户在产品中的行为，助力运营人员优化产品运营策略。
3. 问卷星：在线问卷调研工具，可用于收集用户反馈等信息，为产品优化提供参考。
4. App Store Connect：iOS应用发布管理工具，用于管理iOS应用在App Store的发布等事宜。
...
轻量级devops平台。
1. BigOps：
   - 项目首页：[http://www.bigops.com/](http://www.bigops.com/)
   - 开源地址：[https://github.com/yunweibang](https://github.com/yunweibang)
2. 盘古：国产开源devops平台，小规模开发可以使用。
...
3. LiteOps:
   - 官网：[https://liteops.ext4.cn/](https://liteops.ext4.cn/)
   - 源码：[https://github.com/opsre/LiteOps](https://github.com/opsre/LiteOps)
4. spug: 是面向中小型企业设计的无 Agent的自动化运维平台，整合了主机管理、主机批量执行、主机在线终端、应用发布、任务计划、配置中心、监控、报警等一系列功能。
   - 演示地址：[https://demo.spug.dev](https://demo.spug.dev)
   - 官网地址：[https://www.spug.dev](https://www.spug.dev)
5. CODO是一款为用户提供企业多混合云、一站式DevOps、自动化运维、完全开源的云管理平台、自动化运维平台。
   - 官网：[https://www.opendevops.cn/](https://www.opendevops.cn/)
6. 建木：建木是一个面向DevOps领域的极易扩展的开源无代码(图形化)/低代码(GitOps)工具。
   - 官网：[https://jianmu.dev/](https://jianmu.dev/)
...
1. 白盒测试：知晓被测试对象（如代码、脚本、系统）的内部实现逻辑、代码结构、数据流转细节。
   - 基于内部机制设计测试用例，核心验证逻辑正确性、分支覆盖完整性、异常处理有效性，常用单元测试、静态代码分析、逻辑覆盖等方法，侧重“穿透内部查问题”。 
2. 黑盒测试：不关心被测试对象的内部实现，仅将其视为“黑盒”，通过输入预期数据、观察输出结果及功能表现来验证是否符合需求。
   - 核心关注功能可用性、用户场景适配、输入输出一致性，常用功能测试、场景测试、兼容性测试等方法，侧重“模拟用户验功能”。

白盒测试重要的一个应用点就是系统可用性测试，通过对外暴露测试功能点，主动探测其正确性，例如探活接口。

Uptime Kuma。则是一个轻量级的、面向服务外部可用性的状态监控器。开源：[https://github.com/louislam/uptime-kuma](https://github.com/louislam/uptime-kuma)

特性如下：
1. 监控 HTTP(s) / TCP / HTTP(s) 关键词 / HTTP(s) Json 查询 / Ping / DNS 记录 / Push / Steam 游戏服务器 / Docker 容器的运行时间
2. 支持常用中间件监控，如mysql、Redis、kafka、MongoDB等
3. 精美、响应式、快速的用户界面/用户体验
4. 通过钉钉、电子邮件 (SMTP) 、短信等 90+ 种通知服务进行通知
