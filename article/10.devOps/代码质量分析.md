...
- [SonarQube基础介绍与在代码检测中的应用](https://cloud.tencent.com/developer/article/2129807)
...
主要包括：
1. 代码漏洞扫描：sonar已经实现
2. 开源漏洞检查：依赖引入的漏洞，例如log4j的漏洞。
3. 开源协议检查等：防止类似于GPL的协议的组件对项目的污染。
4. 镜像扫描：针对容器镜像的扫描。
5. 其他：服务器漏洞扫描、web漏洞扫描等，都是需要应用运行后发起的。

![](img/851bfa9e.png)

严重级别：CRITICAL（严重）> HIGH（高危）> MEDIUM（中危）> LOW（低危）。
...
### 4.Trivy
- 开源[https://github.com/aquasecurity/trivy](https://github.com/aquasecurity/trivy)
- 官网[https://trivy.dev/](https://trivy.dev/)

#### 4.1.介绍
Trivy 是由 Aqua Security 开发的开源容器安全扫描工具，旨在快速检测容器镜像、文件系统、Kubernetes 资源、
Git 仓库及基础设施即代码（IaC）配置中的安全漏洞、错误配置和供应链风险。

它以「零配置、开箱即用」为特点，支持扫描操作系统包（如 Alpine、RHEL）和应用依赖（如 npm、pip、Maven）的漏洞，
并能集成到 CI/CD 流水线中，成为 DevSecOps 流程的核心组件。

Trivy 已成为云原生安全领域的主流工具，被 CNCF 收录为毕业项目，并深度集成到 Harbor、GitLab、AWS ECR 等平台。
其开源生态活跃，社区持续贡献新功能，如对 Rust Cargo、Python Poetry 等新兴依赖管理工具的支持。
...
#### 4.2.核心功能与优势
1. 多场景覆盖：支持容器镜像、OCI 制品、WASM 模块、Kubernetes 集群及多云环境的扫描。
2. 高精度检测：针对 Alpine、RHEL 等操作系统的漏洞识别准确率高，并支持 SLSA 合规检测。
3. 灵活集成：可通过 CLI、Docker 镜像或 API 方式嵌入工具链，例如在 GitHub Actions 中配置扫描任务。
   ```yaml
   - name: Trivy Scan
   ...
   ```
4. 离线与加速支持：提供漏洞数据库本地缓存和国内镜像源加速方案，以提升扫描效率。例如：
   ```bash
   export TRIVY_DB_REPOSITORY=https://mirror.ghproxy.com/https://github.com/aquasecurity/trivy-db
   ```
...
其他：推荐直接下载二进制文件，解压即可使用。

# 基本语法：trivy [子命令] [目标] [选项]

1. 扫描容器镜像（最常用）：扫描本地或远程的容器镜像，检测其中的 OS 漏洞（如 Ubuntu 包漏洞）和应用依赖漏洞（如 Java依赖）：
# 扫描本地镜像（例如 nginx:latest）
trivy image nginx:latest

# 扫描远程镜像（例如 Docker Hub 上的镜像）
trivy image ubuntu:22.04

# 扫描私有仓库镜像（需先登录仓库）
trivy image registry.example.com/myapp:v1.0

2. 扫描本地文件系统：扫描主机上的目录（如应用源代码目录），检测依赖漏洞和配置问题：
# 扫描当前目录
trivy fs .
...
3. 扫描 Git 仓库：直接扫描远程 Git 仓库（无需克隆到本地）：
trivy repo https://github.com/example/project.git

4. 扫描配置文件（检测错误配置）：Trivy 可检测 Dockerfile、Terraform、Kubernetes 等配置文件中的不安全设置（如使用 root 用户、权限过高等）
# 扫描 Dockerfile
...
5. 扫描 Kubernetes 资源:检测 Kubernetes 集群中的部署、Pod 等资源的安全问题：
# 扫描所有命名空间的资源
trivy k8s --all-namespaces all

# 扫描指定命名空间的部署
trivy k8s -n default deployment/my-deploy
```

其他选项
- --severity HIGH,CRITICAL：只显示高危（HIGH）和严重（CRITICAL）漏洞
- --ignore-unfixed：忽略未修复的漏洞（只显示有修复方案的）
- --format json：以 JSON 格式输出结果（便于自动化处理）trivy默认提供了多种模版可以使用
- --output result.json：将结果保存到文件
- --no-progress：不显示扫描进度条（适合 CI/CD 环境）
- --vuln-type os,library：指定漏洞类型（os：系统包漏洞；library：应用依赖漏洞）

案例
```shell
1. 只输出高危及以上、有修复方案的漏洞，并保存为 JSON：
trivy image --severity HIGH,CRITICAL --ignore-unfixed --format json --output scan-result.json nginx:latest

2. 扫描Java 的maven的项目。推荐：先编译项目，再扫描整个文件夹。
trivy.exe fs D:\workspace\managing-self\pom.xml  # 不推荐扫描pom文件，推荐扫描编译后的整个项目。
   --format template --template @./contrib/html.tpl    # 使用trivy提供的模版生成HTML
   --severity HIGH,CRITICAL                            # 只显示严重（CRITICAL） 和 高危（HIGH）漏洞
   --output scan-result.html                           # 输出扫描报告
   --ignore-unfixed                                    # 只显示有修复方案的
trivy.exe fs D:\workspace\managing-self --format template --template @./contrib/html.tpl --severity HIGH,CRITICAL  --output scan-result.html
```
...
#### 4.5.如何选择安全的基准镜像
Trivy在其中的作用选择一个安全的基准镜像是保障容器安全的第一步，也是至关重要的一步。 
这遵循“预防为主”的原则。选择安全基准镜像的最佳实践
1. Docker 官方提供了一套名为“Docker Official lmages”的精选镜像，这些镜像是构建安全容器的推荐起点。
2. 选择最小化的镜像:选择只包含应用程序运行所必需组件的最小化镜像。这可以显著减少攻击面。
   - 推荐镜像:alpine、Google的 distroless镜像、Red Hat的 UBI minimal 等。
   - 对比:一个标准的 golang:1.12.9 镜像可能体积巨大且包含上千个漏洞，而 golang:1.12.9-alpine3.9 镜像则体积小得多，漏洞数量也锐减。
3. 使用来自可信来源的镜像:始终从官方或经过验证的发布者那里拉取镜像，例如 Docker Hub 上的官方镜像。避免使用来源不明、长期无人维护的镜像。
4. 保持基础镜像更新:定期更新您使用的基础镜像到最新的稳定版本，以获取最新的安全补丁
5. 以非 root 用户运行:在 Dockerfile 中创建并切换到一个非 root 用户来运行您的应用程序，这是一种重要的安全加固措施。
6. 删除不必要的工具:在构建过程的最后，清理掉所有编译工具、包管理器(如 apt，apk)和其他非运行环境必需的软件。


### 5.Sirius
官网:[https://sirius.publickey.io](https://sirius.publickey.io)
开源地址：[https://github.com/SiriusScan/Sirius](https://github.com/SiriusScan/Sirius)

Sirius Scan是业内首款真正开源的通用漏洞扫描工具，靠信息安全社区的集体技术力，功能上有超越部分商业产品的潜力。
它的核心框架含四大关键模块：漏洞数据库、网络扫描引擎、代理发现机制和自定义评估分析，下文会详解其安装与核心功能，帮安全从业者快速上手。
...
基础功能。
1. 控制面板: 展示系统运行状况指标、实时扫描活动和进度 最新的漏洞发现和趋势等。
2. 控制面板: 展示系统运行状况指标、实时扫描活动和进度 最新的漏洞发现和趋势等。
3. 漏洞查看： 扫描完成后，可在漏洞详情页查看结果，页面支持多种查看、排序及分析漏洞的方式，方便快速梳理信息。
4. 漏洞报告： 选择对应漏洞，会出现相关漏洞报告。
