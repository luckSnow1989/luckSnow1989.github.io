...
1. 基础层：持久化、连接池、分库分表、分布式事务、分布式ID，分库分表等基础知识；
2. 架构层：分组复制、高可用、容灾、扩展性、一致性等数据库架构知识；
3. 内核层：B+树机制，索引机制，ACID机制，MVCC、锁机制、innodb等内核知识；
4. 应用层：用户中心，好友中心，帖子中心，订单中心等不同业务场景下，数据库应该如何设计。
...
- 关系型数据库【RDB】：Oracle、MySQL、Microsoft SQL Server、PostgreSQL、DB2、SQLite。
  - 国产：达梦数据库（DM），人大金仓（KingbaseES），华为 openGauss，阿里 OceanBase，腾讯 TDSQL，TiDB
- 文档型数据库：MongoDB、CouchDB
- kv数据库：Redis、rocksdb、Memcached、etcd、Ehcache
- 列式数据库：ClickHouse、hbase、Cassandra
- 时序数据库：InfluxDB、TDengine（国产）、Apache IoTDB
- 图数据库：Neo4J、GraphDB、Galaxybase（国产）
...
管理方面：
1. 安全：
   - 认证机制：账号密码、LDAP、Kerberos；
   - 访问控制：RBAC、库/表/列等维度的操作权限；
   - 数据加密：传输加密（SSL/TLS）和静态加密（透明数据加密TDE）；
   - 审计监控：操作日志记录和异常行为检测，例如MySQL使用yearning。国产数据库通常自带审计工具。
2. 同步工具：例如MySQL的各种同步工具。OceanBase可以使用OMS、Oracle使用OGG。
3. 高可用：数据库代理是常见的高可用实现方案，还有一些客户端设置的高可用方案。国产数据库通常自带集群部署工具。
4. 性能压测工具：sysbench、benchmarkSQL等
...
5. 分区索引类型：局部索引 和 全局索引。
    - 局部索引：数据和数据在同一个分区中，只能进行分区内的索引查询，无法跨区查询。优点：性能非常好。
    - 全局索引：全局索引是 跨分区 的索引，索引数据可以分布在不同的分区中，可以进行跨区查询。可以做为唯一索引。缺点：性能开销较大。
    - 使用场景，如果查询主要使用分区键，则优先使用局部索引，降低索引的数据量和范围。否则就使用全局索引。
6. 多级分区。OceanBase支持二级分区，开启二级分区的话一级分区必须是range和list中的一种。
7. tablegroup。表组是一个逻辑概念，它描述的是 Table 的一个集合，它们设置相同的分区方式和分区数，表组内的数据都在相同的分区内，提升分区查询性能。
8. 复制表。OceanBase中的一种特殊的表。
...
TiDB 是 PingCAP 公司自主设计、研发的开源分布式关系型数据库。 适合高可用、强一致要求较高、数据规模较大等各种应用场景。

理论基础：作者黄东旭在2012年看到Google的F1/Spanner论文，解决了关系型数据库、弹性扩展以及全球分布的问题，深受启发，决定做一个类似的数据库。

特点：
1. “存算分离”的架构的设计，可按需对计算、存储分别进行在线扩容或者缩容，扩容或者缩容过程中对应用运维人员透明。
2. 100% 支持标准的 ACID 事务。
3. 金融级高可用。基于 Raft 的多数派选举协议可以提供金融级的 100% 数据强一致性保证，且在不丢失大多数副本的前提下，可以实现自动故障转移。
4. 实时 HTAP。作为典型的 OLTP 行存储数据库，同时兼具强大的 OLAP 性能，配合 TiSpark，可提供一站式 HTAP 解决方案，
   一份存储同时处理 OLTP & OLAP 无需传统繁琐的 ETL 过程。
   - 提供行存储引擎 TiKV、列存储引擎 TiFlash。
   - TiFlash 通过 Multi-Raft 协议实时从 TiKV 复制数据，确保TiKV 和 TiFlash 之间数据强一致。
...
重要组件
1. TiDB server。无状态服务，负责接收sql、解析sql、通过PD找到存储数据的tiKv地址。可以无限水平扩展，可以通过负载均衡组件（如 LVS、HAProxy 或 F5）对外提供统一的接入地址。
2. PD server。是整个集群的管理单元，负责管理元数据（哪个key存储在哪个tiKV）、对 TiKV 集群进行调度和负载均衡（如数据的迁移、Raft选举等）、分配全局唯一事务ID。
3. TiKV server。是一个分布式的KV存储引擎。存储数据的基本单位是 Region，副本以 Region 为单位进行管理
   - 每个 TiKV 节点会负责多个 Region，每个 Region 负责存储一个 Key Range的数据。
   - TiKV 使用 Raft 协议做复制，不同节点上的多个 Region 构成一个 Raft Group，互为副本。其中一个为主副本负责写。
   - 数据在多个 TiKV 之间的负载均衡由 PD 调度，这里也是以 Region 为单位进行调度。
4. TiSpark。用于复杂 OLAP 需求的主要组件，Spark SQL 直接解析 TiKV 的数据。作用类似于TiDB server。
5. TiFlash。和普通 TiKV 节点不一样的是，在 TiFlash 内部，数据是以列式的形式进行存储，主要的功能是为分析型的场景加速。


TiKV架构
1. TiKV默认按照范围（Range）自动分片为多个 Region（默认每个 Region 96MB），每个 Region 可跨节点分布。 
  理论上，单表数据量仅受集群总存储容量限制。不受单表数量影响。
2. 也就是说，tidb是按照索引自动分区的。

TiDB支持修改分片策略。
1. Range 分片：适用于时间序列数据（如按时间戳分片），避免热点问题。
2. Hash 分片：适用于高并发写入场景（如用户 ID 哈希），均匀分布负载。
3. 预拆分 Region：通过 PRE_SPLIT_REGIONS 参数提前创建分片，减少写入时的分裂开销。

性能测试对比
1. Sysbench 测试：在相同硬件配置下，TiDB 处理 1 亿行数据的吞吐量是 MySQL 的 3-5 倍。
2. TPC-C 测试：TiDB 7.5 版本在 3 节点集群上达到 120 万 tpmC，远超 MySQL 单机极限。

### 【关系型】PostgreSQL

1. B+Tree 和 hash。与MySQL的功能一直。
2. Gist(通用搜索树，常用语GIS的区域、距离、定位等场景)，使用限制
   1. 有损索引：索引经过计算式有损的，查询结果可能出现误报的行，需要进行二次校验。
   2. Gist的索引数据量很多，构建和更新速度很慢。
3. SP-Gist(空间分区索引)是Gist的变种，支持非平衡的数据结构，内部是将空间划分为不相交的分区进行管理。 
   例如ip地址场景。数据类型为inet或者cidr等ip地址时，SP-Gist会使用基数树（Radix）实现索引，
   Radix是一种专门用于搞笑存储和检索位序列的树形结构，非常适合前缀匹配和包含查询。
4. GIN(倒排索引)。适合精准查询 和 全文检索，索引的数据量很大。通过插件扩展支持中文检索。
5. BRIN(块范围索引)。针对超大数据量的表，将数据划分为多个块，每个块有一个索引，并非正对每行数据创建索引。是数据的逻辑分区（partition是物理分区）
...
- Clickhouse教程[https://book.wdphp.cn/read/clickhouse-tutorial/1535.html](https://book.wdphp.cn/read/clickhouse-tutorial/1535.html)
- ClickHouse介绍[https://zhuanlan.zhihu.com/p/370201180](https://zhuanlan.zhihu.com/p/370201180)
- [ClickHouse 的“独孤九剑”：极速查询的终极秘籍](https://mp.weixin.qq.com/s/t_T2prQuIUpgny4ts8VVaw)
...
倒排索引？term index?sorted Fields?doc values?
1. 倒排索引：通过分词器将文档内容拆分为多个词项，构建词项与ID的映射，实现通过词项进行全文检索的技术。
2. termindex：而现实中文档的数据量是非常大的，产生的倒排索引数据也非常大，不可能将全部数据加载到内存中，所以就有了term index。它将倒排索引公共部分抽离，构建前缀树，极大降低索引体积。加速搜索效率。
3. sorted fields。通过倒排索引获得数据的ID，ID对应的是行式存储结构sorted fields，使用ID进行排序。
4. doc values。很多场景需要使用其他字段进行排序，例如时间、金额、名称等。索引es采用了空间换时间的方案，就是对数据进行冗余，提前生成好基于某列字段的列数存储doc values

数据写入过程？
- 客户端请求 → 协调节点路由 → 主分片写入内存 Buffer 和 Translog → 异步刷新到 Segment（近实时搜索） → 后台合并 Segment（Merge）
- ES中Segment是基本数据结构单位，Segment一旦生成不可修改，如果插入、更新、删除数据，则生成新的Segment。定时将这些Segment进行合并。
- 目的：不浪费服务器CPU资源、提升数据读取效率。

Lucene 的 Segment 合并（Merge）对性能有什么影响？如何优化？
- Merge 会消耗大量 I/O 和 CPU，导致写入延迟。
- 优化方法：调整合并策略（Tiered Merge Policy）、限制合并线程数、使用 SSD。

Elasticsearch 的 Translog 和 WAL（Write-Ahead Log）的作用是什么？
- Translog 就是一种WAL实现方案，用于故障恢复，保证未刷新到磁盘的数据不丢失。日志中记录持久化磁盘的状态，故障恢复时会读取状态，重新执行数据的操作。

近实时（NRT）搜索是如何实现的？
- 写入数据首先进入内存 Buffer，通过refresh_interval（默认 1 秒）刷新到 Segment，生成新的不可变 Lucene 段，此时数据可被搜索。

Elasticsearch 如何处理分布式一致性（如写冲突）？
- 基于版本号（_version）的乐观锁机制，主分片在写入时检查版本号，冲突时客户端需重试或自定义冲突解决策略。

脑裂问题？
- 通过discovery.zen.minimum_master_nodes（7.x 之前）或基于 Raft 的选举机制（7.x 之后）避免。

为什么 Elasticsearch 默认使用 BM25 作为相关性评分算法？与 TF-IDF 的区别是什么？
- BM25 考虑了文档长度归一化和词频饱和，更适合长短文档混合的场景；TF-IDF 对高频词权重过高，易受长文档影响
...
#### 1.mysql proxy
MySQL Proxy是一个处于你的client端和MySQL server端之间的简单程序，它可以监测、分析或改变它们的通信。它使用灵活，没有限制，
常见的用途包括：负载均衡，故障、查询分析，查询过滤和修改等等。MySQL Proxy就是这么一个中间层代理。
简单的说，MySQL Proxy就是一个连接池，负责将前台应用的连接请求转发给后台的数据库，并且通过使用lua脚本，可以实现复杂的连接控制和过滤，从而实现读写分离和负载平衡。
对于应用来说，MySQL Proxy是完全透明的，应用则只需要连接到MySQL Proxy的监听端口即可。当然，这样proxy机器可能成为单点失效，但完全可以使用多个proxy机器做为冗余，
在应用服务器的连接池配置中配置到多个proxy的连接参数即可。MySQL Proxy更强大的一项功能是实现“读写分离”，基本原理是让主数据库处理事务性查询，让从库处理SELECT查询。
数据库复制被用来把事务性查询导致的变更同步到集群中的从库。

#### 2.Mycat
Mycat是基于阿里开源cobar演变而来，对cobar的代码进行了彻底的重构，使用NIO重构了网络模块，并且优化了Buffer内核，增强了聚合，Join等基本特性，
...
#### 2.Mysql Router
MySQL Router是MySQL 官方发布的数据库中间件，是处于应用client和dbserver之间的轻量级代理程序，它能检测，分析和转发查询到后端数据库实例，并把结果返回给client。
是mysql-proxy的一个升级产品。
主要功能：
1. 读写分离
2. 从库负载均衡
...
[项目从 MySQL 切换 PostgreSQL，踩了太多的坑！！](https://mp.weixin.qq.com/s/nChijd_FJ3pR66XyN2vEBg)
