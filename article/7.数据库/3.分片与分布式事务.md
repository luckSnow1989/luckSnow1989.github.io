...
2. 一致性hash算法：通过虚拟节点，可以在扩容后降低迁移的数据量。扩容后也需要迁移数据。
3. 查分法：多是SaaS服务采用，为不同的用户定制个性化分片策略。
4. 基因算法：在需要两个信息进行绑定的时候使用。缺点扩容后，原有的分片字段需要重新计算，目前使用率较低。

### 1.7.分区

mysql分区： https://blog.csdn.net/qq_35190486/article/details/108758205
...
自己实现复杂查询的方案：
1. 确认分表后，执行多个where .. order by .. limit x, 然后在程序中进行结果合并，排序等。
2. 记录查询第一页时用于排序的字段最大/最小值。比如order by time。翻页查询的时候，作为查询条件。
3. 根据业务情况，禁止跨表差
4. 使用es、clickhouse、等，将历史数据提前进行聚合。例如APP的数据，甚至可以缓存到客户端
5. 创建多个维度的冗余表，比如客户id分表和商店id的分表
6. 使用分布式数据库。效果好些，但是成本高。

### 1.9.数据聚合问题

- https://blog.csdn.net/majunzhu/article/details/100163523
...
- 方案1：数据冗余，提前聚合数据，就是创建一张维护不同的，但是字段相同的表，数据进行同步。
- 方案2：历史数据一般不会发生变化，可以提前聚合。例如日分表聚合为月分表。用于数据统计。
- 方案3：数据同步至es、hbase、clickhouse等大数据平台，可以提升聚合速度。
- 方案4：提前在客户端聚合。例如美团APP的订单列表页面。
...
Sharding-JDBC 执行原理：https://www.cnblogs.com/dalianpai/p/12312640.html

确实用起来比较复杂，但是对于大量的分库分表的场景，简单的操作是非常方便的
实际工作中，简单的单表查询，与简单的跨表查询，使用Sharding jdbc。
对于复杂的表连接、分页查询、多字段排序等。使用的原生的SQL。消除Sharding jdbc的不稳定性，与复杂场景的规则配置。

分布式事务：sharding-jdbc支持三种事务类型，local、xa、base:https://shardingsphere.apache.org/document/current/cn/features/transaction/
- local:本地事务，各自数据库管理各自的事务
- xa：利用数据库的xa协议实现，两阶段提交
- base柔性事务:就是集成Seata，支持高并发和长事务
...
- 为什么需要分布式ID以及分布式ID的业务需求：https://www.jianshu.com/p/1b5472fe4e77
- 分布式ID之UUID适合做分布式ID吗： https://www.itqiankun.com/article/1565060584
- 分布式ID之数据库自增ID实现分布式ID：https://www.itqiankun.com/article/1565142723
- 分布式ID之Redis集群实现的分布式ID：https://www.itqiankun.com/article/1565227901
- 分布式ID之雪法算法Snowflake实现分布式ID：https://www.itqiankun.com/article/1565747019
...
- 雪花算法 Snowflake：完美的方案，推荐使用美团的算法（强依赖时间，所以最好关闭机器的时间同步）
- ULID：ULID = 时间戳(48位) + 随机数(80位)，再通过 Base32 Crockford编码后，转化为一个26字符的可读字符串。
   主要用于代替UUID，提升数据的顺序性和时间属性。对时间依赖性弱，通过随机性，降低冲突概率。
...
- 方案1【不推荐】：将用于生成id的服务器的时间回拨能力关闭，也就是时间自动同步功能。此方案存在问题，例如很多定时任务中间件，会要求C/S两端的时间差在一定的范围内，可能导致任务失败。
- 方案2【不推荐】：遇到回拨后直接报错，交给人工处理，缺点：官方方案，不符合实际操作，但是美团的leaf就是这个策略
- 方案3【不推荐】：小步长的回拨，比如3ms内，系统可以sleep3ms等待。缺点：大流量服务会造成一定程度的请求阻塞。
- 方案4【推荐】：增加1000长度的数组，保存最近的值，当出现回退时，与数组中的值进行比较，每比较一次，ID+1，直到找到ID小于当前值的位置。
...
- Seata：阿里实现的分布式事务框架。
- TCC：ByteTCC、himly、tcc-transaction
...
RocketMQ实现原理：事务消息也是采用了2PC的思想。
1. 生产者向 Broker 发送半消息，随后执行本地事务。
2. 根据本地事务执行结果，生产者通知 Broker 提交或回滚事务消息。
3. 若因异常导致消息状态未及时处理，Broker 会定期检查半消息状态，调用生产者的事务消息监听器询问本地事务状态，
   生产者根据实际情况返回相应状态，以确保消息的发送和本地事务的执行达到最终一致性。

核心代码： TransactionalMessageCheckService。这里涉及到3个topic
1. RMQ_SYS_TRANS_HALF_TOPIC保存半消息，Broker每60s取一次消息，判断是否有需要会校验的消息。
2. RMQ_SYS_TRANS_OP_HALF_TOPIC保存完成半消息的数据。要是判断需要校验的话，就向生产者发送消息进行校验，成功的话就将消息转移到真正的topic中。
...
- Seata提供的分布式事务解决方案：http://Seata.io/zh-cn/docs/overview/what-is-Seata.html

支持的事务模式：
1. 提供XA。强一致性 
2. TCC。最终一致性，需要开发。
3. AT【推荐】。强一致性，二阶段优化版，无侵入(需要使用注解)
4. saga【推荐】。需要开发，无锁，最终一致性。 Saga 模式是一种长事务解决方案，它采用状态机来定义事务流程， 
  将一个大的业务流程拆分成多个子业务流程（子事务），每个子事务都有对应的补偿操作

#### 6.2.1.AT模式

AT模式实现原理： https://www.jianshu.com/p/566613e336d7

1. Seata服务启动后，应用必须引入Seata提供的DataSourceProxy，这样Seata就可以拦截到所有的数据库操作。
2. 应用程序开启全局事务后，每个事务都会向Seata注册全局事务，并且会生成一个全局唯一的XID。
3. Seata会主动创建一个全局锁(不操作数据库操作，仅用于应用之间通信)，然后本事务涉及的其他几个应用向Seata发起分支事务申请。
4. 别的事务开启时会向Seata注册分支事务，如果发现该数据已经被全局锁锁定了，就等待（成功或超时）。

前提要求：
1. 使用事务的多个应用必须都引入了Seata提供的DataSourceProxy，并开启全局事务。
2. 每个需要参与全局事务的数据库中都需要提前创建好一张UNDO_LOG表，用于记录事务执行的快照（前镜像 与 后镜像），方便事务回滚和提交。

AT模式脏读或脏写的问题（因为AT是一种TCC的模式，应用层面看到的是事务没有提交，其实数据库已经提交了）：
- 解决方案。首先我们没办法在db层实现，所以我们只能在应用层实现。然后我们在使用Seata的时候，其实已经将DataSource做了一层代理了。
    所以我们的思路就是强制让这几个表操作的所有SQL都必须走分布式事务。

比如：@ GlobalLock + @ Transactional  或 @GlobalTransaction。
这样Seata维护者全局事务和全局锁，就不会出现问题。其中本地事务使用使用@GlobalLock，他只校验锁，不会生成新的全局锁

#### 6.2.2.全局锁
Seata全局锁实现原理：https://blog.csdn.net/qq_38685072/article/details/117619323

特指AT模式，在AT模式下，Seata默认使用全局锁的方式来保证数据的一致性。
