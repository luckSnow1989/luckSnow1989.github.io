...
- [MySQL 全文索引详解：从原理到实战，一文搞懂！](https://www.cnblogs.com/springsongs/p/18976186)
...
MySQL能有现在的地位，InnoDB功不可没。 在机械硬盘的时代，顺序读写的BTree家庭就非常的合适。
而随着固体硬盘和分布式技术的发展，LSM tree就越来越被关注起来。
...
索引是帮助数据库高效获取数据的数据结构。使用更小的数据，更快的检索速度。
...
在LSM-tree 有三个部分，
1. MemTable：Memory Mutable Table 内存可变表
1. Immutable MemTable：内存不可变表
2. SSTable：Sorted String Table  有序字符串表
...
hash-skiplist的索引既有hash索引，又有skiplit的二分索引，针对于有明确key或教完整key前缀的查询，如果要进行全表扫描，会非常耗费内存及低性能，因为要产生临时的有序表； 
hash-linklist也是一样的道理。

为什么不是B树呢？因为B树的以页为单元进行存储的，内存操作的时候会影响效率。
而OceanBase中MemTable和SSTable都是以数据块进行维护的，一个数据块2MB，数据块内使用16KB的数据微块。MemTable由HashTable和B+tree组成。
HashTable用于单行查询。B+tree用于范围查询。并且方便进行数据块的维护。
...
1. 利用tree的二分查找特性，提高查询效率。而B+tree相比B-tree，他的叶深度更低，查询快。
2. 全表扫描性能好，非叶子节点只保存了索引数据和指针，没有数据，所以查询数据时只需要遍历整个非叶子节点。
3. io性能好，非叶子节点只保存了索引数据和指针，可以保存更新的索引数据
4. 底层数据保存的结构为双向链表，天然支持排序。
...
顾名思义：最左优先，以最左边的为起点任何连续的索引都能匹配上。右边索引遇到范围查询(>、<、between、like)就会停止匹配。
...
特殊场景：mysql优化器可能使用了错误的索引，导致性能降低，这时可以是用force强制走某个索引。

Where a= and b= and c in()。abc都有各自的索引，最终查询的时候，怎么走的索引。

这个是根据实际的数据情况，走的索引不一样

- 可能性1：数据库的查询优化器，会选择数据区分度最高的一个。剩下两个查询条件不走索引，只使用条件查询
- 可能性2：当单个索引无法满足需求时，且索引合并的成本估算合理时，会开启索引合并。前两个索引的结果marge后where结果
...
上面的连接中有:delete where id=一条数据后，是怎么一个过程。
不是真实删除，只是在底层打了一个删除的标识，不释放物理空间，为了不影响表的性能，可以被后面的数据覆盖。
...
INSERT INTO articles (title, body) VALUES
    ('MySQL Optimization', 'MySQL provides full-text indexing for efficient keyword searching.'),
    ('Database Index', 'Using B-Tree index for exact match and range queries is efficient.'),
    ('Elasticsearch Integration', 'For complex text search, Elasticsearch works better than built-in fulltext.'),
    ('Chinese Search', 'MySQL native fulltext search doesn’t support Chinese well.'),
    ('Boolean Mode', 'Use + and - in boolean mode to refine your search.'),
    ('Stopwords and Token Size', 'Understanding stopword lists and min token size is critical.'),
    ('Performance Tips', 'Fulltext index uses more space and slower insert performance.'),
    ('Fulltext vs Hash', 'Hash index supports equality, while fulltext helps in content-based retrieval.'),
    ('Fulltext Score', 'We can sort results by relevance using MATCH...AGAINST scoring.'),
    ('Stemming Feature', 'Searching for "running" also matches "run" in MySQL.');
...
最佳实践
```sql
1. 停用词（Stopwords）：默认情况下，MySQL 会忽略一些无实际意义的常用词（如 "the"、"and"、"a"）。这可能导致你搜索的词根本没被索引！
    查看当前的 InnoDB 停用词表：SELECT * FROM information_schema.INNODB_FT_DEFAULT_STOPWORD;
    自定义停用词表：           SET GLOBAL innodb_ft_user_stopword_table='your_db/your_table';
                   
2. 词干提取（Stemming）。MySQL 会对词语做简化处理，例如：搜索running， 结果包括run。
    但要注意，这种词干提取是基本级别，远不如专业搜索引擎（如 Elasticsearch）。

3. 复数形式支持，例如：搜索book，匹配结果books。但是child，匹配不到children
    但是复杂或不规则复数形式匹配能力较弱。

4. 最小词长限制：默认情况下，过短的词无法被索引：例如 InnoDB	innodb_ft_min_token_size=3
    修改这些设置后需 重建全文索引。                          
...
6. 相关性评分：排序利器， 分数越高表示相关性越强，适合用于搜索结果排序或打分推荐。
...
1. 性能提示
    全文索引占空间较大
    批量更新/导入数据后需注意延迟更新问题
    可使用 OPTIMIZE TABLE 手动重建全文索引:OPTIMIZE TABLE articles;

2. 中文处理能力较弱。需要指定分词器
使用 ngram 分词器（MySQL 自带的 ngram parser）来生成固定长度的 token（常用 2-3），这样对短语中间匹配、部分匹配都能有倒排索引支持。
...
3. 不支持多字段全文模糊匹配。（当然可以见多个索引，但是需要的资源太多了）。
```
