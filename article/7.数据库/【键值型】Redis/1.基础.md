...
- v4.0：
  - 引入多线程进行异步任务，体现为提供了一些非阻塞的命令，如UNLINK、FLUSHALL ASYNC、FLUSHDB ASYNC等，任务会在后台线程中执行，不会阻塞主线程事件循环。
  - 布隆过滤器。安装时需要添加扩展module
- v5.0：Streams。是sub/pub的进阶产品，解决不能保存数据的问题，相当于内存版的kafka
- v6.0：以前版本的Redis一直是单线程的reactor线程模型，直到6.0版本Redis正式引入多线程网络模型。
  用于后台任务、网路io处理(socket读写、协议解析、请求响应)，命令执行还是单线程。
   dubbo也采用类型的模型，单线程selector、I/0线程负责读写请求、worker线程负责业务逻辑。
- v8.0：
  - 优化内存结构。单机性能从十万提升到百万。核心优化了内存的分配机制（k和v一次性分配） 
  - redisVL支持AI工作流。
...
```shell
redis-benchmark.exe -h 127.0.0.1-p 6379 -c 10 -n 10000 -q -t -k get
...
13.redis-cli命令。前缀为redis-cli -h 127.0.0.1 -p 6379 -a 密码 
    执行redis基础命令，例如 set k v等
    执行lua脚步：--eval /tmp/script.lua 参数1 参数2
    连续统计模式：--stat， key数量、内存使用大小、客户端连接数量、阻塞的客户端数量、请求次数、客户端链接次数。
    扫描大key。--bigkeys。底层使用SCAN实现不会全表扫描。
    扫描热key。--hotkeys。必须开启lfu策略。
    监控实例的延迟：--latency
```
...
语法
```shell
# offset 就是第几位,string 最大512MB，所有offset最大就是 512*1024*1024*8=536,870,912*4 （5.37*8=42.96亿） 
# value只有0和1，否则就报错了
...
适用场景
1. 社交应用：用户可以根据地理位置发现附近的其他用户，进行好友推荐或匹配。
2. 物流配送：根据配送员或车辆的实时位置，找到附近的订单或配送需求。
3. 共享服务：共享单车、共享汽车等应用可以通过 Redis Geo 快速定位最近的车辆。
...
<p style="color: red">实现原理</p>
在redis server端的redisClient对象中，存在一个队列，用于保存事务的命令。
当开启事务后执行写入命令并不会立即执行，而是将名称缓存起来，当执行exec命令后再真实执行。且这个过程中，如果连接断开会导致事务丢失。

### 4.2.Pub/Sub

Redis提供简单的发布订阅模式，采用广播模型，一次性发送，不具备失败重试、重新消费、数据持久化等功能，消息错过了就收不到了。

实现原理：
1. 订阅：redis保存一个订阅列表，当客户端连接到redis订阅频道后，对在订阅列表添加一个频道，并将客户端连接缓存进去。
2. 发送消息：当发布消息时，会读取这个列表中缓存的客户端连接，依次调用。

使用办法:订阅支持正则表达式，例如: 有两个频道 news.it, news.et。同时订阅多个可以使用 news.[ie]t 或者 news.* 
...
它以追加的方式存储数据，任何添加到 Redis Stream 的数据都是不可变的。 这种特性使 Redis Stream 非常适合处理有序且连续的数据流。因为Redis对外提供的操作都是纯内存操作，因此速度是非常快的。

特点：适用于中等规模的工作负载，不合适高吞吐场景（每秒5000左右）。

内部结构
- 数据结构：内部通过 Radix 树结构来存储条目。
- 特点：添加条目的操作时间复杂度是 O(1) ，因为它仅涉及数据的追加。同样，从流中读取条目是一个时间复杂度为 O(K) 的操作，其中 K 是 ID 的长度。
由于 K 是常数，因此实际读取可以视为常数时间操作。

```json
1. 消息存储：有序字典（类似基数树 + 哈希表）
Stream = {
  "消息ID1": { "field1": "value1", "field2": "value2", ... },
  "消息ID2": { "field1": "value3", "field2": "value4", ... },
}

2. 消费进度：每个 Stream 可以创建多个消费者组，每个组的元数据包括：
  a)last_delivered_id：组内当前消费到的最新消息 ID（即下一条待消费的消息起点）。
  b)消费者集合：记录组内的消费者（如 consumer-1、consumer-2）。
  c)Pending 消息索引：追踪已分发给消费者但未确认的消息
  Pending = {
    "消费者ID": {
      "消息ID1": { "投递次数", "首次投递时间" },
      "消息ID2": { "投递次数", "首次投递时间" },
...
生产者与消费者。
- 多个生产者应用程序可以向同一个流中添加数据，同时也可以有多个消费者应用程序从流中读取数据。这种设计实现了生产者与消费者之间的解耦，使双方可以独立运行，互不依赖。
- 消费者采用了 消费者组的概念。同一个消息 在 一个消费者组中只会被消费一次，不同的消费者组也都会消费一次。和MQ的概念一样。
  区别是：因为消费者采用拉取的方式，所以消费者组中的消费者可以同时进行消费，每次拉取完成后提交消费记录到Redis中。

优势
- 高性能：由于 Redis Streams 是内存存储的，它能够实现低延迟和高吞吐量。此外，其日志结构化的数据模型也确保了高效的读写操作。
- 灵活的数据模型：Redis Streams 采用无模式设计，客户端可以根据需要存储任意键值对，非常灵活。
- 与其他 Redis 数据结构的集成：Redis Streams 可以与 Redis 中的其他数据结构（如列表、集合和哈希）无缝结合，增强数据处理能力。
- 易于使用：Redis Streams 提供了多种编程语言的 API、命令行界面（CLI）和 SDK，使开发者能够方便地使用和集成 Redis Streams。
- 时间序列数据：每个流条目都带有时间戳 ID，条目按顺序存储，特别适合时间序列数据的管理与分析。

缺点
- 持久性：由于数据主要存储在内存中，存在在断电或宕机时数据丢失的风险。如果应用场景对数据持久性有较高要求，Redis Streams 可能不是最佳选择。
- 消息保留时间有限：受限于内存资源，Redis 无法长时间保留所有消息。需要客户端主动清理，或设置自动清除策略。
- 复杂事件处理能力有限：与 Apache Kafka 或 Apache Flink 等系统相比，Redis Streams 不支持复杂的事件处理功能，功能相对基础。

使用场景：
- 任务队列：对于一些简单的任务，例如发送电子邮件或短信通知，Redis Streams 可以提供高效的解决方案。同时通过增加消费者组中的消费者数量，可以轻松提升其处理能力。
- 会话管理和活动跟踪：适合管理短期用户交互，例如跟踪用户的在线/离线状态。由于 Redis 是内存存储，操作速度非常快，性能优异。
- 轻量级事件溯源：可用于事件记录后续异步处理的场景，即先将事件写入流中，之后进行异步处理。其内存存储特性使其特别适合低延迟的事件处理，但不适合高吞吐量的事件。
- 实时分析：非常适合实时性要求高、数据保留周期短的分析场景。例如，展示 CPU 使用率等实时指标。
...
64432.99 requests per second
====== LRANGE_100 (first 100 elements) ======
49825.61 requests per second
====== LRANGE_300 (first 300 elements) ======
27800.95 requests per second
====== LRANGE_500 (first 450 elements) ======
22026.43 requests per second
====== LRANGE_600 (first 600 elements) ======
18618.51 requests per second
====== MSET (10 keys) ======
62695.92 requests per second
```

注意： 网络io的影响是非常大的。本地压测吞吐量可以达到6W，通过局域网的话，可能降低到1W，再通过域名的话，可能只有7，8千了。
...
- RedisFront。开源的Redis桌面客户端。[https://gitee.com/dromara/RedisFront](https://gitee.com/dromara/RedisFront)
