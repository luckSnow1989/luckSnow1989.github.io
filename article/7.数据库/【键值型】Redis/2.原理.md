...
- Runid:每次Redis节点启动的时候，随机生成16为字符串，作为节点的唯一标识
...
主要流程如下
1. 主观下线(SDOWN)：以每秒钟一次的频率向的master发送PING命令，检测节点是否正常。如果master超过down-after-milliseconds(默认30s)设置的时间内没有回复，则该哨兵认为master主观下线。
2. 客观下线(ODOWN)：当一个哨兵发现master主观下线后，它会询问其他哨兵，是否也认为master主观下线。当有足够数量的哨兵（法定人数）达成一致，则该master被标记为客观下线，触发故障转移。
   期间若没有足够数量的 哨兵同意 Master下线 或者 master重新恢复响应，Master的客观下线状态就会被移除。
3. 领头哨兵选举：为了避免多个哨兵同时执行故障转移，造成混乱，需要从哨兵中选举一个领头哨兵（leader），由它来负责故障转移的流程。
   选举过程基于Raft算法的变种，选举过程：哨兵之间发送消息，内容为自己的runid，谁最先发送的就是领头哨兵。
4. 新master选举：领头哨兵会从所有的从节点中选择一个最合适的从节点，作为新的master。
   1. 确认有效的slave
   2. 找出slave的优先级,配置文件配置，默认都一样
   3. 找出slave复制偏移量最大的，info命令获得
   4. 找出runid最小的，启动时生成的，就是最先启动的
5. 故障转移：当新master选举出来后，领头哨兵会向所有的从节点发送命令，让它们与新master建立复制关系，更新master信息。
6. 通知客户端：哨兵通过pub/sub方式通知所有订阅了哨兵的客户端修改master信息。客户端接收到信息后创建新连接。
...
Redis cluster中如果主节点发生故障，因为节点都是对等的，所以需要节点发现该节点故障后，会进行投票选举，选举使用的还是Gossip

1. 疑似下线：节点之间通过Gossip协议发送ping，如果某个主节点 cluster-node-timeout（默认 15 秒）内未响应，则认为是疑似下线。
2. 正式下线：发现问题后节点会向集群中所有节点广播该master的状态。当超过半数的主节点达成一致后，则标记该主节点为正式下线。
3. 重新选举：该下线master下的从节点进行选举。选举规则同哨兵模式。优先级最高->复制偏移量最大->runid最小。
4. 故障转移：赢得选举的从节点，执行 SLAVEOF NO ONE命令，切换角色为master。
5. 集群配置变更：向集群中所有节点广播新master信息。故障期间可能发生slot的重新分配，这时就需要再次进行重分配slot。
6. 客户端变更：1、执行命令时发现集群变更，更新客户端集群信息。2、redis通过发布订阅通知。
...
- 目的：linkedlist中每个节点都是一个节点，连续性差，造成大量内存碎片，导致内存利用率不高。
     而quicklist极大的降低节点的数量，很大程序提升了集合元素的连续性。
...
ziplist设计初衷是为了降低内存占用，减少内存碎片，通过时间换空间。
但是因为结构紧凑，所以当数据变更/插入时，可能引发内存realloc，原数据进行内存拷贝，造成内存连续更新。
...
多层双向链表组成，最底层保存全部数据，越往上层，保存的数据越少。
...
3.新节点的层数是怎么确定的？跳表最多可以创建多少层？
新节点使用一个类似于连续抛硬币的随机算法确定层数，最低也有一层，即level0层。
最多应该是32层吧，一个宏定义的。

4.跳表的创建过程？
创建过程就是寻找节点的过程，从最高层level层开始找每层level链表的终点(最大的小于目标值的那个节点)，最终找到目标节点，
途中每层的终点记录下来，更新指针和跨度，一个过程顺便记录每层跨度和，最后累加起来就是目标节点的rank。
...
采用渐进式 rehash 原理：每次处理客户端请求的时候，先从 表1 中第一个索引开始，每次访问都迁移部分数据到2中，逐步将所有数据迁移过去，
就这样将 rehash 分散到多次请求过程中，避免耗时阻塞。
...
如何发现热key
1. redis 6以上，提供工具。redis-cli --hotkeys，基于scan实现。（需要开启LRU或者LFU）。
2. 低版本可以使用redis monitor命令，但是对性能影响大，只产生基本数据，还需要进一步统计。
3. 代理层统计。例如codis、使用代理收集（Twemproxy），提供相关的功能。
4. 客户端统计。客户端收集（有赞方案）；京东的hotkey，通过客户端收集定时上传到服务端，统计是否为热key，实现本地缓存。

如何发现大Key
1. redis 6以上，提供工具。redis-cli --bigkeys，基于scan实现。
2. 低版本使用memory usage [key]命令，通过遍历所有的key实现检测。
   redis-cli --scan | while read key; do echo "key: $key, memory usage: $(redis-cli memory usage $key) bytes"; done
3. redis-rdb-tools。是一个开源的 Redis RDB 文件分析工具，可以解析 Redis 的 RDB 文件，找出其中的大 key。
4. 慢查询。起到一定的参考作用。

解决方案：redis集群模式下拆分多个key（拆分）、降低访问次数（多级缓存）
...
1. 设置过期时间，且需要在一条命令中执行。为什么这样？防止异常退出后，后续程序无法解锁。合成一个命令也是这个问题
2. value设置一个唯一值，什么这样？防止误删锁。
...
- RedissonLock(非公平锁、可重入锁)：hash，field是当前锁生成的随机值，value是加锁的次数。
