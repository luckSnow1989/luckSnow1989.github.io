...
1. ThreadLocalMap的维护。这个问题解决不了，尽量不要设置太多的ThreadLocal就行
...
（1）CountDownLatch的计数器只能使用一次。而CyclicBarrier的计数器可以重置。
...
（3）CountDownLatch会阻塞调用await方法的线程，CyclicBarrier只会阻塞子线程所有的子线程，等待主线程完成触发的方法。
