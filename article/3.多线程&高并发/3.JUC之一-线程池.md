...
newCachedThreadPool工作原理（主要基于队列SynchronousQueue实现。）
...
public void execute(Runnable command) {
    int c = ctl.get();
    //第一步：判断工作线程是否小于核心线程（线程池一开始就把所有核心线程创建出来）
    // 当已经有足够的核心线程的时候，就不会执行这里了。
    ...
    //第二步：当已经有足够的核心线程的时候，提交的任务就直接加入任务队列
    // 因为核心线程调用了workQueue#take或者poll方法阻塞等待新任务提交
...
方案2：hippo4j。【推荐使用】。一款强大的动态线程池框架，解决了传统线程池使用存在的一些痛点比如线程池参数没办法动态修改、不支持运行时变量的传递、无法执行优雅关闭。
