...
- 上锁：一旦出现并发竞争，会进行一次CAS，成功就拿到锁，失败直接触发 “锁膨胀”，升级成重量级锁。
（注意：轻量锁没有自旋，网上很多说自旋的，是跟重量锁）
...
但如果锁竞争激烈，轻量锁就不但有互斥的开销，甚至比重量锁更慢。

#### 3.4.4.重量锁

轻量锁设计的目的是解决“低竞争、短持有”的场景，如果采用CAS自旋非常消耗CPU，所以就不自旋，直接升级。
即使jdk提供的各类Lock也没有采用自旋的方式，更何况是底层的synchronized。

思考：既然有了轻量锁，为什么还要有重量级锁？
- 因为轻量锁只是为了解决“低竞争、短持有”的场景。面对高并发场景还得使用重量级锁。
重量级是调用计算机底层实现的，重量级中存在队列。会将竞争关系的线程阻塞排队，但也是非公平的

自适应自旋(jdk8对重量锁的优化):
- 采用 “自适应次数”，目的是减少线程阻塞开销。
