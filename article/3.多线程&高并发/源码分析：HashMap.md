...
- 1.8加入红黑树。当链表的长度大于8的时候，将链表的数据保存到红黑树；当红黑树的长度小于6的时候，又会将数据转为链表。红黑树是一种特殊的二叉树，用高效的检索效率
- 1.7先扩容再插入，1.8先插入在扩容
...
总结：当多个线程同时进行扩容操作，假设线程 A 执行到一半被挂起，此时部分元素已经转移；线程 B 完成扩容，链表顺序反转。
线程 A 恢复执行，继续用头插法转移元素，由于之前部分元素已被反转，就可能形成环形链表。
例如，原本链表是 A->B，线程 B 反转成 B->A，线程 A 再操作就可能使 A 指向 B 同时 B 又指向 A，形成环链，后续查询可能陷入死循环。
...
于是，当我们的线程一调用到，HashTable.get(11)时，悲剧就出现了——InfiniteLoop。
...
<p style="color: red">hashMap 加载因子，设置的过程？</p>
...
<p style="color: red">hashMap如何快速的保存 100 个元素？使用多线程插入一定比单线程插入慢吗？</p>
单线程时，影响HashMap性能最大的因素是扩容，减少或避免扩容能极大的提高性能。所有可以设置集合容量，计算方式是 元素数量/0.75 + 1。
这样集合不进行扩容。 元素=100， 计算值=134, 初始化容量=256, threshold=192
多线程在这样简单的操作中，是远不能提高性能的。线程上下文切换，线程池内部逻辑等。

<p style="color: red">hashMap 设置容量为100时，内部真实的容量是多少？</p>

设置100，初始化容量是128，加载阈值96。如果元素数量也是100的话，添加数据的时候，还会出现一次扩容。

<p style="color: red">为啥使用红黑树而不是用二叉树或者平衡树？</p>

红黑树的时间复杂度是 O(LogN)，比较链表的O(N)性能好。
二叉树虽然也可以使用二分查找，但是作为基础的数据结构，可能出现数据倾斜，单边数据过多，退化为链表
红黑树是一种特殊的平衡树，有更多的读写特性在里面，性能更快一些。
