...
CRC(Cyclic Redundancy Check)，即循环冗余校验，是一种根据网络数据包或电脑文件等数据产生简短固定位数校验码的快速算法（散列函数），
主要用来检测或校验数据传输或者保存后可能出现的错误。CRC利用除法及余数的原理，实现错误侦测的功能，具有原理清晰、实现简单等优点。

若信息码字为111000110，生成多项式G(x)=x^5+x^3+x+1，则计算出的CRC校验码为11001
1. 将生成多项式的系数作为除数。取多项式最高幂次数+1=6，作为校验码的位数。然后再取多项式个项的幂次数5、3、1、0次幂，表示6位二进制中为1，其余为0，得到111000。
2. 将信息码左移生成多项式的最高幂次数 5 位，作为被除数111000110+00000。
3. 执行模2除法，即异或操作。
4. 等到 5 位余数即为校验码。11001

![](img/jiami/d0dbe0b7.png)
...
4. deflate。Deflate是一种无损数据压缩算法，结合了LZ77和Huffman 编码，被广泛用于 HTTP 数据传输（deflate 和 gzip）、ZIP 文件格式和 PNG 图片格式。
5. LZMA。高压缩比、慢速度，字典很大。应用：7z、xz、7-Zip 压缩包。
6. BZIP2。基于 Burrows–Wheeler 变换，压缩比高于 DEFLATE，速度一般。应用：Linux 软件包、tar.bz2。
7. LZ4。极快压缩 / 解压，压缩比一般，追求速度。应用：Redis、Kafka、游戏资源。 
8. Snappy。Google 开发，速度优先，压缩比适中。应用：Hadoop、BigQuery、ClickHouse。 
9. Zstandard（Zstd）。Facebook 推出，在压缩比和速度之间平衡极好。 应用：Linux 内核、MySQL、ZIP 新版、很多后端存储。

有损压缩算法：特点：丢弃人眼 / 耳不敏感信息，无法完全还原，但体积大幅减小。
1. 图片
   - JPEG：基于 DCT 变换，最常用照片压缩。
   - WebP：Google 格式，比 JPEG 更小，支持无损 / 有损。
   - HEIC / HEIF：苹果手机默认，压缩效率更高。
   - AVIF：基于 AV1 编码，新一代高压缩图片格式。
2. 音频
   - MP3：最普及的音频有损压缩。
   - AAC：比 MP3 更高效，苹果、YouTube 主流。
   - OGG Vorbis：开源免费，音质优秀。
   - WMA：微软音频格式。
3. 视频
   - H.264 (AVC)：目前最通用视频编码。
   - H.265 (HEVC)：同画质下体积减半。
   - VP9：Google 开源，YouTube 大量使用。
   - AV1：新一代开源免专利，压缩比极高。
   
常见压缩算法选型
- 普通文件压缩：ZIP(GZIP/DEFLATE)
- 极致压缩比：7z(LZMA)
- 速度优先：LZ4 / Snappy
- 综合最优：Zstandard
- 图片：WebP / AVIF
...
2. GZIP
    ...
    - 应用：在http请求中，deflate只对请求数据进行压缩。而gzip是对deflate的改进，在deflate基础上增加文件头、校验和等额外信息。其兼容性更好。
3. RAR
