...
在kafka中没有日志层面的数据压缩能力，压缩主要由消息的生产者提供，生产者支持压缩类型：none（默认值，即无压缩）、gzip、snappy、lz4或zstd。
压缩针对的是整批数据，因此批处理的效率也会影响压缩率（批处理越多，压缩效果越好）。

设计目的：Kafka 的压缩流程是 “生产者压缩 → Broker 存储压缩数据 → 消费者解压缩”，这种设计将计算开销（压缩 / 解压缩）分散到生产者和消费者端，
避免 Broker 成为性能瓶颈，同时最大化了传输和存储效率。

优势：
1. 减少网络传输带宽（压缩后数据量通常可降低 50%-90%）
2. 降低磁盘存储空间占用； 
3. 减轻 Broker 的网络和磁盘 I/O 压力。


#### 4.5.1.核心指标对比表

| 指标                | Gzip                | Snappy             | LZ4                 | Zstandard（Zstd）     |
|---------------------|---------------------|-------------------|---------------------|----------------------|
| 压缩比          | 高（1:5~1:10）       | 中低（1:2~1:5）    | 中等（1:3~1:7）      | 极高（1:5~1:10+，可配置） |
| 压缩速度        | 慢（~100MB/s）       | 快（~500MB/s）     | 极快（~1GB/s）       | 极快（~2GB/s，快速模式） |
| 解压缩速度      | 慢（~300MB/s）       | 极快（~2GB/s）     | 极快（~4GB/s）       | 极快（~5GB/s）        |
| 内存占用        | 低                   | 中低                | 低                  | 中高（可配置降低）     |
| 流压缩支持      | 是                  | 是                | 是（帧格式）        | 是                   |
| 多线程支持      | 部分实现            | 部分实现          | 原生支持            | 原生支持             |
| 硬件加速        | 无特殊优化          | 无特殊优化        | Intel QAT可选       | Intel AVX2/AVX-512优化 |
| 典型应用场景    | 日志存储、归档       | Hadoop、Spark     | Kafka、Redis        | 实时压缩、大数据管道  |

#### 4.5.2.详细特性
1. Gzip
   - 优势：压缩比最高，适合长期存储或带宽受限场景（如日志归档），兼容性极强（几乎所有系统原生支持）。
   - 劣势：压缩/解压缩速度最慢，CPU占用高，不适合实时数据流处理。
   - 典型场景：Nginx日志压缩、HTTP响应压缩、Linux系统tar.gz归档。
2. Snappy
   - 优势：压缩/解压缩速度极快，解压缩速度可达2GB/s以上，内存占用低，适合追求吞吐量的场景。
   - 劣势：压缩比最低，存储空间占用较多，不适合对存储成本敏感的场景。
   - 典型场景：Google Bigtable、HBase数据存储，Spark Shuffle过程数据压缩。
3. LZ4
   - 优势：
       - 速度极致：解压缩速度可达4GB/s（LZ4帧格式），是当前最快的压缩算法之一。
       - 低延迟：压缩开销极小，适合对实时性要求高的系统（如消息队列）。
       - 多线程支持：原生支持多线程并行压缩，充分利用多核CPU。
   - 劣势：压缩比中等，极端场景下可能不如Zstd。
   - 典型场景：Kafka消息压缩（默认推荐算法）、Redis缓存压缩、游戏数据传输。
4. Zstandard（Zstd）
   - 优势：
       - 平衡型王者：压缩比接近Gzip，同时压缩速度超过LZ4（快速模式下），解压缩速度可达5GB/s。
       - 可配置性强：支持1~22级压缩级别（1为最快，22为最高压缩比），适应不同场景需求。
       - 硬件优化：针对现代CPU架构（如AVX-512）做了深度优化，多核利用率高。
   - 劣势：压缩时内存占用较高（尤其在高压缩级别），需要较新的硬件或驱动支持。
   - 典型场景：Facebook数据中心压缩、Elasticsearch索引压缩、实时数据管道（如Flink）。

#### 4.5.3.场景选择建议
1. 存储成本优先：Zstd（高压缩比）> Gzip（兼容性强）
2. 实时性优先：LZ4（极快解压缩）> Snappy（高速处理）
3. 平衡场景：
    - 消息队列（Kafka/RocketMQ）：LZ4（默认）或Zstd（新版推荐）
    - 大数据处理（Hadoop/Spark）：Snappy（Shuffle）或Zstd（存储）
    - 日志归档：Gzip（压缩比）或Zstd（兼顾速度）
4. 硬件考量：
    - 老旧服务器：Snappy/LZ4（低资源消耗）
    - 新服务器（Intel Xeon等）：Zstd（利用AVX-512加速）

#### 4.5.4.性能测试参考数据

Intel i7-8700K

| 算法       | 压缩速度（MB/s） | 解压缩速度（MB/s） | 1GB数据压缩后大小 |
|------------|------------------|------------------|------------------|
| Gzip（6级）| 120              | 350              | 200MB            |
| Snappy      | 550              | 2200             | 450MB            |
| LZ4（fast） | 1100             | 4100             | 320MB            |
| Zstd（1级） | 1800             | 5200             | 280MB            |
| Zstd（22级）| 300              | 5200             | 180MB            |


通过上述对比可知，没有“绝对最优”的算法，选择需根据业务场景的核心诉求（存储、速度、资源）灵活决策。
当前趋势是Zstd凭借综合优势逐渐成为主流，尤其在新版大数据框架（如Kafka 3.0+）中已作为推荐算法。
...
Kafka 事务消息的核心作用是实现跨分区、跨会话的原子性操作，确保多个消息的发送、消费者 Offset 提交等操作要么全部成功，
要么全部失败，从而解决分布式系统中因网络波动、节点故障等导致的 数据不一致 问题。

场景：从数据库读取数据并写入 Kafka，需保证数据库操作与 Kafka 消息发送的原子性（如：扣减库存后发送订单消息）。

避免重复消费与消息丢失
- 生产者端： 事务消息确保消息发送的幂等性（通过序列号去重），避免网络重试导致重复写入。
- 消费者端：事务提交成功后，消费者以 read_committed 模式读取消息，避免读取未提交的事务消息。

事务流程【生产者】
1. 初始化事务： 生产者调用 initTransactions()，向协调器注册 transactional.id。
2. 开启事务：beginTransaction()，生成唯一事务 ID（包含 epoch 防止僵尸实例）。
3. 发送消息：消息暂存于缓冲区，标记为 “未提交”，Broker 不会立即写入日志。
4. 提交事务：commitTransaction() → 协调器向所有涉及 Partition 发送提交标记 → Broker 将消息标记为可见。
5. 异常处理：若超时或失败，协调器触发事务回滚（消息标记为废弃）。

事务流程【消费者】设置读取消费的隔离级别：
- read_uncommitted（默认）：消费者可读取所有消息（包括未提交的事务消息）。
- read_committed：仅读取已提交的事务消息，未提交的消息对消费者不可见。
...
buffer.memory   生产者缓冲区大小，单位B
batch.size      批处理大小，增大可提高吞吐量，单位B。生产者端默认批量发送，即当消息量达到阈值才会触发发送，频繁触发会影响性能。
acks            消息确认模式，-1 表示需要确认所有副本都写入成功，1 表示只需确认一个副本写入成功，0 表示无需确认。

压测结果
1000000 records sent, 139606.310205 records/sec (136.33 MB/sec), 
334.05 ms avg latency, 426.00 ms max latency, 343 ms 50th, 401 ms 95th, 419 ms 99th, 423 ms 99.9th.

吞吐量：每秒发送约 13.96 万条消息（136.33 MB/s），可以开启压缩提升吞吐量
延迟（Latency）：平均延迟 334.05 ms，最大延迟 426 ms，50%、95%、99%、99.9%分位上的延迟。
    延迟偏高，可能由以下原因导致：
    1.生产者 batch.size 配置过小（如默认 16KB），导致频繁发送；
    2.linger.ms 配置为 0（默认），未启用批量发送；
    3.单机测试时磁盘 I/O 或网络带宽成为瓶颈。

将batch.size设置为32768 （32KB）后，压测结果。吞吐量提升到18.12万(提升28.9%)，平均延迟1.79(降低185倍)。
1000000 records sent, 181257.930034 records/sec (177.01 MB/sec), 
1.79 ms avg latency, 325.00 ms max latency, 2 ms 50th, 3 ms 95th, 7 ms 99th, 27 ms 99.9th.
...
--fetch-size: 每次拉取的数据大小（字节）
--messages: 消费的消息总数
--threads: 消费者线程数

压测结果
start.time, end.time, data.consumed.in.MB, MB.sec, data.consumed.in.nMsg, nMsg.sec, rebalance.time.ms, fetch.time.ms, fetch.MB.sec, fetch.nMsg.sec
2025-06-30 17:52:21:505, 2025-06-30 17:52:23:001, 976.5674, 652.7857, 1000005, 668452.5401, 326, 1170, 834.6730, 854705.1282

1.消费 100 万条消息耗时约 1.496 秒，吞吐量为：652.79 MB/s（数据量）   66.85 万条 / 秒（消息数）
2.重平衡时间 326 ms：消费者组分区分配耗时，若频繁发生会影响稳定性（正常单次重平衡应 < 100ms）。
3.拉取时间 1170 ms：从 Broker 拉取数据的总耗时，对应拉取吞吐量：834.67 MB/s（数据量）  85.47 万条 / 秒（消息数）
```
...
案例1：[服务上限再提升35%！去哪儿如何将Kafka性能榨到极致？](https://mp.weixin.qq.com/s/OS7eish57AI_WLW2WWmFtw)

优化思路：随着业务增长，KAFKA 数据也会不断膨胀，集群压力随之增长，如 IO、网卡、闲置率、磁盘存储、CPU 等。需要权衡一个压缩值，既不影响数据延迟，也能提升集群容量，保障业务线的增长。

优化步骤：完善集群监控，加入指标：客户端读写次数，闲置率，生产条数与生产次数的比值，流量与生产次数的比值（计算单条消息大小），IO，流量等基础指标等。

具体操作：1、选择合适的压缩算法（降低带宽压力、降低存储成本）；2、提升batch.size（消息合并、批量发送、降低发生频率）


案例2：如何降低生产者端的消息延迟。
- 延迟：从消息发送到 ACK 接收的总耗时（单位：毫秒）
- 影响因素：ACKS的设置直接影响。可靠性越强延迟越高。

优化方向:
1. 降低生产者延迟
   - 合理设置batch.size（16~32KB之间），linger.ms>0时生效，生产者会积攒多条消息一起发送，减少网路IO次数。
   - 设置 linger.ms=0，不等待批处理，>0生产者会积攒多条消息一起发送，延迟低了网路IO次数却多了，会降低broker的吞吐量。
   - 设置 ACKS=0或1。
   - 优化序列化，使用protobuf代替JSON，提升序列化效率。
2. 降低网路延迟
   - 缩短生产者和broker之间的物理距离（在同一个zone中）
   - 增加带宽（至少使用万兆网卡）
   - 启用压缩，降低网路传输量
3. 降低broker延迟
   - 使用SSD替换HDD
   - 调整 log.flush.interval.messages（避免频繁刷盘）
   - 增加 ISR 副本数（但 acks=all 时会增加复制延迟）
...
<p style="color: red">kafka高性能揭秘</p> 

[零拷贝-实现原理](/article/16.Linux/1.Linux理论)


1. 磁盘顺序写：采用顺序追加写入的方式，可以有效减少IO寻址的时间。顺序写入也是页缓存适用的场景。
2. sendfile。当消费者从 Broker 拉取消息时，采用sendfile实现零拷贝，将磁盘文件读到内核缓冲区后，直接转到socket buffer进行网络发送，避免内核态切换到用户态。
   如果当时数据已经加载到页缓存中，则直接从页缓存读取数据。降低进一步的IO操作。
3. 页缓存。利用操作系统的页缓存，提升读写效率。顺序写和sendfile都是页缓存适用的场景，避免了磁盘 I/O 操作。
4. mmap。mmap适合小量数据的读写操作。主要应用于索引文件的读写，加快索引的检索。
5. 其他:批量发送、数据压缩、topic分区等。

![](img/kafka/8c66a59c.png)

<p style="color: red">kafka和RocketMQ的零拷贝</p> 

对于绝大部分的中间件，都使用了这类技术到达高性能的。
- Kafka：PageCache 写日志、 sendfile 传输消息、 mmap管理索引文件
- RocketMQ：mmap写日志和管理索引文件、sendfile传输消息、PageCache 预读机制（Broker 启动时加载文件，避免运行时抖动）。
    除此之外RocketMQ 堆外内存。默认采用 堆外内存（Direct Buffer） 作为网络 I/O 的缓冲区，避免数据在 JVM 堆内存与本地内存之间的拷贝。
    减少 JVM 堆与操作系统之间的内存拷贝开销，进一步优化传输效率。

<p style="color: red">RocketMQ低延迟如何实现</p>
RocketMQ 是适用于 金融级高可靠、低延迟的场景。通过PageCache 预读机制 和  堆外内存使用，RocketMQ 达到了极致的性能。
实现消息达到broker服务器中，从socket buffer 到 page cache ，再到 socket buffer 的过程。
整个过程中，数据只在内核空间中进行拷贝，避免了用户态和内核态的切换，降低消息延迟。

<p style="color: red">RocketMQ零拷贝的缺点</p>

- Kafka 直接对缓存进行追加写入，适用于大量数据的写入场景。
- RocketMQ 选择了 mmap + write 作为零拷贝方式，更适合业务级消息这种小数据块/高频率的 IO 传输。

RocketMQ 零拷贝的缺点：使用mmap会占用系统内存（虚拟内存），可能导致系统内存使用率过高。如果这时产生大量的新分配需求（大量写入，乱序读取）或者缺页中断，
可能会触淘汰算法进行内存回收，此时可能会产生抖动，写入会有短时的毛刺现象。

通过网上研究测试表明
1. 如果读取的数据小于4kb的时候，使用mmap的性能效率比sendfile高，当读取数据大于4kb的时候，sendfile的效率比mmap高； 
2. 如果写入的数据包小于64kb的时候，mmap的性能效率比sendfile高，当写入数据包大于64kb的时候sendfile的效率比mmap高。
...
<p style="color: red">rocketmq 和 kafka 对比？</p>

一、核心架构对比

| 维度    | Kafka                               | RocketMQ                |
|-------|-------------------------------------|-------------------------|
| 设计目标  | 高吞吐、日志处理、流数据管道                      | 金融级可靠性、事务消息、低延迟         |
| 存储模型  | 分区（Partition）顺序追加日志                 | 队列（Queue）+ 消费位点（Offset） |
| 元数据管理 | 早期依赖 ZooKeeper，新版 KRaft 去 ZooKeeper | 轻量级 NameServer（无状态）     |
| 副本同步  | ISR 机制（动态同步副本集合）                    | 主从同步/异步复制（可配置）          |
| 消费模式  | 消费者组（Consumer Group）                | 消费者组 + 集群/广播模式          |

二、功能特性对比

| 功能    | Kafka             | RocketMQ            |
|-------|-------------------|---------------------|
| 消息顺序性 | 分区内严格有序           | 队列内严格有序             |
| 消息过滤  | 无原生过滤，需消费者自行处理    | 支持 Tag 过滤、SQL92 表达式 |
| 延迟消息  | 不支持（需外部实现）        | 支持 18 个延迟级别（1s-2h）  |
| 事务消息  | 支持（需事务型生产者和消费者）   | 原生支持两阶段提交事务消息       |
| 消息回溯  | 按 Offset 回溯       | 支持按时间戳/Offset 回溯    |
| 死信队列  | 无                 | 支持自动重试 + 死信队列       |
| 消息轨迹  | 需集成外部工具（如 Zipkin） | 原生支持消息轨迹追踪          |

三、性能对比

| 维度   | Kafka                 | RocketMQ              |
|------|-----------------------|-----------------------|
| 吞吐量  | 单机百万级 TPS（批量处理优化）     | 单机约 10 万级 TPS（更侧重低延迟） |
| 写入延迟 | 较高（批量刷盘策略）            | 更低（同步刷盘 + 内存映射）       |
| 消费延迟 | 基于 Pull 模式，延迟可控       | 长轮询（Push 模式体验）        |
| 资源消耗 | 高（依赖外部组件，如 ZooKeeper） | 低（NameServer 轻量级）     |

四、高可用与容灾

| 机制    | Kafka                 | RocketMQ                |
|-------|-----------------------|-------------------------|
| 副本同步  | ISR 动态选举（优先保证可用性）     | 主从同步 + 故障自动切换（Dledger）  |
| 数据可靠性 | `acks=all` 保证不丢失      | 同步刷盘（Broker） + 同步复制（主从） |
| 故障恢复  | Controller 选举新 Leader | 主节点故障后，从节点自动升主          |
| 数据一致性 | 最终一致性（ISR 机制）         | 强一致性（Dledger Raft 协议）   |

五、生态系统与扩展性

| 维度    | Kafka                         | RocketMQ             |
|-------|-------------------------------|----------------------|
| 流处理集成 | 完善（Kafka Streams、Flink、Spark） | 有限（需结合外部流处理框架）       |
| 云原生支持 | 主流云厂商托管服务（如 Confluent）        | 阿里云、腾讯云等国内云厂商支持      |
| 监控运维  | 需依赖第三方工具（Prometheus+JMX）      | 自带 Dashboard（运维更简单）  |
| 社区生态  | 全球活跃（Apache 顶级项目）             | 国内主导（阿里开源，Apache 项目） |

六、适用场景

Kafka 更优场景：
- 大数据管道：日志采集、实时数仓（与 Flink、Spark 无缝集成）。
- 流处理：复杂事件处理（CEP）、实时计算（Kafka Streams）。
- 高吞吐量：广告点击流、用户行为跟踪（批量写入场景）。

RocketMQ 更优场景：
- 金融交易：强一致性要求（如分布式事务消息）。
- 电商业务：订单状态同步、库存扣减（低延迟 + 事务保障）。
- 复杂路由：按 Tag 或 SQL 过滤消息（如营销消息精准推送）。

七、选型建议
- 选择 Kafka：需要高吞吐、流处理生态、国际化团队协作、复杂日志场景。
- 选择 RocketMQ：需要事务消息、低延迟、国产化技术栈、金融级可靠性。

八、高级对比（面试深挖）
1. 存储机制差异：
    - Kafka 使用分段日志（Segment） + 索引文件，适合顺序读写。
    - RocketMQ 使用 CommitLog 统一存储 + 队列索引，减少随机 IO。

2. 零拷贝实现：
    - Kafka 使用 `sendfile` 系统调用（减少内核到用户态拷贝）。
    - RocketMQ 使用内存映射（MappedByteBuffer）+ PageCache 优化。

3. 事务消息实现：
    - Kafka：两阶段提交（事务协调器 + 事务日志）。
    - RocketMQ：半消息（Half Message） + 本地事务表 + 回查机制。

4. 负载均衡策略：
    - Kafka：消费者组内分区重平衡（Rebalance）。
    - RocketMQ：支持平均分配、一致性 Hash 等策略。
