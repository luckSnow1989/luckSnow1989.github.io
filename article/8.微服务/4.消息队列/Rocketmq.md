...
- [使用自带工具进行性能测试](https://cloud.tencent.com/developer/article/1498084)
- [性能测试](https://www.cnblogs.com/chongyou/p/12391103.html)
...
对拉取结果进行处理，如果有消息，反序列化，再进行一步tag的比较去重。
broker的ConsumeQueue中保存tag的hashcode值， 所以consumer的此步去重是保证消息的准确性。
...
commitlog：commitlog就是来存储所有的元信息，包含消息体，类似于Mysql、Oracle的redolog,所以主要有CommitLog在，Consume Queue即使数据丢失，仍然可以恢复出来。
...
1. 生产者：同步发送消息，配合重试机制。或者直接使用事务消息。
2. Broker：刷盘机制（可以同步刷盘）+ 同步机制（主从同步复制） 
3. 消费者：手动提交使用+消息重试机制。

rocketmq的方式比kafka要简单不少，但是刷盘和同步机制会大大性能服务的性能（本来就不高）
而且rocketmq只能通过配置文件修改刷盘与同步方式的。所以对数据要求严格的场景，我们会在生产者端记录消息到数据库，之后去生产者端进行数据校验。


<p style="color: red">负载均衡策略</p>
默认采用的是平均分配算法来进行负载均衡。
...
<p style="color: red">当消费负载均衡consumer和queue不对等的时候会发生什么？</p>
Consumer和queue会优先平均分配，如果Consumer少于queue的个数，则会存在部分Consumer消费多个queue的情况，如果Consumer等于queue的个数，
那就是一个Consumer消费一个queue，如果Consumer个数大于queue的个数，那么会有部分Consumer空余出来，白白的浪费了。

<p style="color: red">使用不当造成脏页</p>

系统日志，通过mq进行采集，可能操作mq性能降低。日志这种不重要的数据和主业务一起进消息队列容易抢pagecache，
之前我们用rocketmq接日志的时候就出现过pagecache busy的问题，八成是日志量太大，造成过多的脏页了，这也导致了主业务受得了影响，
最后用disruptor做buffer，grpc做异步通讯专门处理日志，不得不说http2的性能加纯异步编程，太强悍了。

pageCache介绍：https://blog.csdn.net/Cecilia3333/article/details/103786654

kafka对pageCache的使用：https://blog.csdn.net/gx11251143/article/details/107620259

<p style="color: red">commitlog为何设置为单个文件？</p>

在早期版本中，RocketMQ 的所有主题消息都存储在同一个 CommitLog 文件中。这种设计虽然简化了全局管理，但在多主题高并发场景下可能导致以下问题：
1. 锁竞争：多个主题的消息写入同一文件时，容易产生锁冲突，影响写入性能。
2. 文件膨胀：单一文件过大时，文件管理（如滚动、删除）的效率降低，且可能增加磁盘 IO 延迟。
3. 消费干扰：不同主题的消费请求可能互相影响，例如某个主题的大量读取操作可能导致其他主题的缓存被挤出。

RocketMQ 在 4.5.0 ，每个主题的消息存储在独立的 CommitLog 文件中。
1. 减少锁竞争：每个主题的 CommitLog 文件独立，不同主题的写入操作互不干扰，降低了锁竞争，提升了并发写入性能。
2. 优化文件管理：单个主题的 CommitLog 文件体积更小，文件滚动和删除操作更高效，尤其适合多主题场景。
3. 提升局部性：同一主题的消息集中存储，消费者读取时可利用更高效的缓存策略，减少跨主题的 IO 干扰。
4. 扩展性增强：按主题拆分后，可针对不同主题的存储需求进行独立配置（如文件大小、存储路径等），提升了系统的灵活性。

<p style="color: red">kafka和rocketmq的索引结构？</p>

Kafka 使用稀疏索引的主要原因是在存储成本、写入性能和查询效率之间进行权衡。
采用稀疏索引，每4096条消息存储一个索引。读取日志文件时，采用二分查找的方式，加快查找速度。

RocketMQ 的索引ConsumeQueue采用了稠密索引的设计，为了提升查询效率，索引使用定长存储，每个索引固定20B。包括如下信息：
1. 消息偏移量（Offset）	long 8B。消息在CommitLog中的物理偏移量，用于直接定位消息。
2. 消息长度（Size）	int	4B。消息在CommitLog中的总长度（包括头部和消息体）。
3. 消息 Tag 的哈希值（TagsCode）	int	4B。消息Tag字段的哈希值，用于消息过滤（如消费者通过Tag订阅消息）。

通过ConsumeQueue的第一个offset就可以计算出对应Offset所在的位置，实现O(1)的查询效率，直接定位到消息在commitlog中的位置。

IndexFile（可选索引）：RocketMQ 还提供了IndexFile（索引文件），用于根据消息的Key查询消息。
IndexFile 采用哈希表结构，记录Key的哈希值、消息偏移量和时间戳等信息，支持快速的 Key 查找。