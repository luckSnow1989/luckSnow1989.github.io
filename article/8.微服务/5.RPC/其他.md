...
Feign 是 Netflix 开源的声明式 HTTP 客户端，集成于 Spring Cloud 生态，通过注解和接口简化服务调用，支持负载均衡、熔断降级和服务发现。
其优势在于与 Spring Cloud 深度整合，功能全面，但依赖第三方库，需额外配置。

@HttpExchange 是 Spring Framework 6（对应 Spring Boot 3）引入的注解，旨在通过声明式接口定义 HTTP 服务。
它减少了对外部依赖的需求，与 Spring 原生功能（如 WebClient）无缝集成，代码更简洁，性能更优。


使用方法与@RequestMapping，@GetMapping等注解基本一致，只是替换为HttpExchange相关的注解，消除了以往的歧义。
