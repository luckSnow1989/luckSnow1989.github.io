...
2. 客户端每5s一次向服务端发送心跳，nacos每3s检查所有的连接，如果超过20s没有心跳，则主动向客户端发送连接检查，出现异常时也会主动断开连接，剔除服务实例。
...
Nacos 虽然使用了raft协议，但是并没有leader的概念，不需要进行选举。 raft的主要作用是帮助节点见数据同步。

CP和AP模式是不能切换的，因为二者使用的场景不同。只能在永久数据和临时数据中自动进行选择使用。
...
2. 配置中心：使用Derby采用的CP模型，其他数据库不用AP和CP模型。
...
2. SDK与节点间：为了保证配置中心的高可用，并未采用raft算法，而是通过对比MD5来实现，如果不一致SDK就拉取节点中的最新配置。
    Derby数据库需要使用raft协议，因为是嵌入式数据库，需要保持各个节点之间的数据一致性。
