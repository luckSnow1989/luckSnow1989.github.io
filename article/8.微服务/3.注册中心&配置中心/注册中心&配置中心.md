...
- ectd只能使用第三方提供的可视化客户端:[https://github.com/evildecay/etcdkeeper](https://github.com/evildecay/etcdkeeper)
...
# 压缩内存，只保留最近1000个版本。
etcdctl --endpoints=http://127.0.0.1:2379 compact $((rev-1000))
...
优点：
1. 相比于zk，etcd 使用起来要简单很多。不过要实现真正的服务发现功能，etcd 还需要和其他工具（比如 registrator、confd 等）一起使用来实现服务的自动注册和更新。
2. 相比于zk，etcd 采用 raft 协议作为一致性算法，比基于Paxos的zab协议实现更加简单，选主阶段时间更短。

缺点：etcd 默认只保存 1000 个历史事件，所以不适合有 高频、大量更新操作的场景，这样会导致数据的丢失。[ETCD的内存问题与最佳实践](https://coolshell.cn/articles/22242.html)
1. 内存消耗高。
   - etcd采用raft同步数据时，默认会保留最近5000条数据在内存中（不会被淘汰长期占用内容）。
   - treeIndex 是基于内存版 btree 库实现，会加载全部数据，构建索引。
   - 通过mmap将整个blotdb文件加载到虚拟内容中
...
quota-backend-bytes: 8589934592  # 8GB触发压缩
...
关键指标监控清单
1. 数据库大小：正常<500MB，告警>2GB，紧急>5GB
2. 碎片化率：正常<30%，告警>50%，紧急>80%
3. 压缩频率：建议每5-10分钟自动压缩一次
4. 历史版本数：建议保留1000-5000个版本
