...
1. 实例状态。枚举：InstanceStatus
    ...
    UNKNOWN         未知状态
2 实例操作类型。参考：PeerAwareInstanceRegistryImpl.Action
    Heartbeat               心跳检查
    Register                注册
    Cancel                  取消
    StatusUpdate            状态变更
...
> PS：eureka是基于AWS云的网络配置实现的。在公有云的维度，一般情况下，region之间网络是不通，同一个region下的zone之间网络互通。
...
1. registry，一级缓存，核心注册表，数据来源：服务注册/续约/下线等实时更新。UI界面直接使用registry；
   - 类型：ConcurrentHashMap<String, Map<String, Lease<InstanceInfo>>>
   - 实现类：AbstractInstanceRegistry
2. ReadWriteCacheMap，二级缓存，缓存时间180秒，缓存失效后，从一级缓存中同步数据更新。
   - 类型：LoadingCache<Key, Value> 
   - 实现类：ResponseCacheImpl
3. ReadOnlyCacheMap，三级缓存，客户端缓存，Eureka Client查询时生成，默认每30s。通过定时任务轮询判断是否需要移除缓存。
   - 类型：ConcurrentMap<Key, Value>	
   - 实现类：ResponseCacheImpl	

![](img/eureka/41776c1f.png)
...
```java
public class Value {
    private final String payload; // 序列化为json的应用级数据（不是实例级别的）
    private byte[] gzipped;       // 压缩后的json数据（不是实例级别的）
}    
```

因为客户端查询数据，需要对数据进行序列化，当Eureka注册应用实例数量到达10~30K，就会导致客户端与服务端频繁的序列化与反序列化，
造成不必要的CPU消耗，所以采用缓存序列化好的数据，降低频繁序列化的资源消耗，提升并发处理能力。
同时也降低并发读写时带来的频繁的锁竞争，所以采用了类似于读取分离的设计。
...
默认配置下延迟计算：
1. 客户端获得实例变化存在延迟，极端情况可能是：30(ribbon更新)+30(客户端拉取)+30(三级缓存更新)+180(二级缓存更新) = 270s。
2. 如果被调用方没有使用平滑升级(被动剔除服务)，极端情况可能是：270(上面基础)+ 60 * 2(两次主动清除下线90s没有心跳的实例)= 390s。

服务端优化
1. eureka.server.useReadOnlyResponseCache=false。关闭三级缓存，直接读取ReadWriteCacheMap
2. eureka.server.responseCacheUpdateIntervalMs。二级缓存更新时间，默认180s，可以降低到10s
...
经过以上优化，极端情况下延迟：10(ribbon更新)+10(客户端拉取)+10(二级缓存更新) + 7 * 10(两次主动清除下线65s没有心跳的应用)= 100s。

除此之外还有非常多的参数都可以进行优化，但是要根据自身情况，并且无论进行怎么优化，延迟都是存在的。

<p style="color: red">二次开发可优化点？</p>

1. 关闭三级缓存后。增加一个队列，每次当注册表发生变化的时候，将变化的数据放到队列中，开始一个任务线程，读取队列数据，同步到二级缓存中，实现准实时更新。
2. 优化客户端拉取注册表。增加项目维度的管理工具，每次值拉取已经订阅的项目列表，减少无效数据的拉取。
...
同步核心逻辑：PeerAwareInstanceRegistryImpl#replicateToPeers

```java
private void replicateToPeers(Action action, InstanceInfo instanceInfo, boolean isReplication) {
    if (isReplication) {
        // 避免重复同步（若当前操作本身来自其他节点的同步）
        return;
    }
    // 获取所有peer节点,向每个节点发送同步任务
    for (final PeerEurekaNode node : peerEurekaNodes.getPeerEurekaNodes()) {
        replicateInstanceActionsToPeers(action, instanceInfo, node);
    }
}
```

触发时机：当接口接受请求时同步调用。
![](img/eureka/232dfe07.png)
...
eureka并未提供监控指标，需要通过自定义指标来实现。并且grafana也没有监控面板，需要我们自己定义。
...
public class EurekaConfig {
    ...
    // 示例：统计注册服务数量
    @Bean
    public MeterBinder registeredServicesCounter(EurekaClient eurekaClient) {
        return registry -> {
            Counter counter = Counter.builder("eureka.registered_services")
                    .description("Number of registered services")
                    .register(registry);
            Applications applications = eurekaClient.getApplications();
            counter.increment(applications.size());
        };
    }
}
```
...
可以自定义的指标包括
1. 服务注册与实例状态指标
   - eureka_server_instance_counts{status="UP"}：状态为UP的服务实例数量（核心指标，反映健康服务数量）。
   - eureka_server_instance_counts{status="DOWN"}：状态为DOWN的服务实例数量（异常服务监控）。
   - eureka_server_instance_counts{status="STARTING"}：处于启动中状态的服务实例数量。
2. 注册中心集群同步指标
   - eureka_server_peer_replication_events_total：Eureka集群节点间的复制事件总数（反映集群数据同步频率）。
   - eureka_server_peer_replication_failures_total：集群复制失败次数（监控集群一致性问题）。
3. 服务续约与下线指标
   - eureka_server_renewal_rate：服务续约成功率（正常应接近1.0，低于阈值可能有服务异常）。
   - eureka_server_evictions_total：因未续约被剔除的服务实例总数（异常增长可能意味着服务稳定性问题）。
4. Eureka Server自身性能指标
   - jvm_memory_used_bytes：Eureka Server的JVM内存使用量（监控内存泄漏）。
   - http_server_requests_seconds_count{uri="/eureka/apps"}：服务注册/查询接口的请求次数（反映Eureka的负载）。
   - process_cpu_usage：Eureka Server的CPU使用率（监控服务器负载）。