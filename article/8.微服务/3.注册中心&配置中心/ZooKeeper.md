...
sessionTimeOut -- 会话超时时间。以毫秒为单位，默认2000。客户端和服务器端之间的连接通过心跳包进行维系，如果心跳包超过这个指定时间则认为会话超时失效。
...
<p style="color: red">zk客户端连接时，是如何进行负载均衡的</p>

StaticHostProvider类的构造方法，使用Collections.shuffle();将zk地址打乱，然后从0开始选择。如果客户端连接时出现问题，
比如重建客户端，则调用next方法，连接到下一个地址

<p style="color: red">客户端连接丢失了，常见的原因有啥</p>
网络、zk某个节点出现问题（可能是flower或者master）、zk数据过多导致gc时间过长，心跳超时，导致断开。

<p style="color: red">客户端连接zookeeper集群，已经写入了部分临时节点数据，如果客户端断开连接，并立刻连接其他zookeeper节点，请问之前写入的临时节点数据是否已经丢失了？</p>
临时节点数据是否丢失取决于客户端是否在sessionTimeout（默认2秒）内恢复会话。客户端连接时会产生一个sessionId，
sessionId会被同步到集群各个节点。如果在超时时间内连接到别的节点则数据不会丢失。