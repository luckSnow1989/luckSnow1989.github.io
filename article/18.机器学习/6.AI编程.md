...
| 维度                |  Claude Code         |  openCODE  |  Cursor         |  Trae              |
| ----------------- |------------------------|--------------|-------------------|----------------------|
...
模型分配策略：不同子智能体可配置不同模型，充分发挥各模型的特点：
...
- 开源优秀案例：https://skillsmp.com/
- 开源优秀案例：https://github.com/techthird/skills/tree/main/skills/mysql-to-java-ddd

#### 3.3.安装工具

openskills：安装在claude根目录的就是全局skills，安装在项目根目录的就是项目级别的skills。
- 安装：npm i -g openskills    
- 初始化：openskills install anthropics/skills
- 同步在线的skills：openskills sync


Superpowers：本质是一个技能库框架，采用ai cli插件的方式安装。每个 Skill 就是 一套行为规则 + 用例 + 推荐步骤，让 AI 在对应阶段自动调用合适策略。
而 Superpowers 是一个面向 AI 编程代理的 技能库框架，最初流行于 Claude Code / Codex 等环境，用于让 AI 遵循专业开发流程和规范，而不是随机生成代码。
...
opencode目前最强形态是配合它的外挂插件oh-my-opencode。不仅完全免费开源，更集成了Claude Code和 AmpCode的所有核心优势，甚至在某些维度做的更好。

支持的模型非常多，并且有一些直接作为免费的提供：

![](./img/6.AI编程/19535979610200.png)

插件安装：https://github.com/code-yeongyu/oh-my-opencode
