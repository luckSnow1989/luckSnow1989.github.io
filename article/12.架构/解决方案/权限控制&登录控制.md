...
单点登录（SSO）是一种认证机制，使得用户只需要一次登录，就可以访问多个相互信任的系统或应用服务。

技术选型方案

| 方案                                 | 技术框架                                       | 特点           | 适用场景                        |
|------------------------------------|--------------------------------------------|--------------|-----------------------------|
| Cookie + Session                   | Spring Session                             | 轻量、本地适用      | 子系统同域部署，中小型系统，不要求复杂授权逻辑     |
| OAuth2 / OIDC                      | Spring Authorization Server、Keycloak、Auth0 | 标准协议、最通用     | 互联网企业/SaaS统一身份认证，多语言、多端系统对接 |
| JWT + 网关鉴权                         | Spring Security + JWT + Gateway            | 微服务适配、高性能    |  微服务架构，高并发、无状态服务集群  |
| CAS（Central Authentication Service） | Apereo CAS                                 | 老牌稳定、学校/政府常用 | 政企内网 / 传统系统                 |

实现原理
- Cookie + Session：多个子系统共享统一的 Session 存储（如 Redis）。
- OAuth2 / OIDC：使用授权服务器颁发AccessToken/IDToken，使用标准协议完成认证授权。
- JWT + Gateway：用户登录后，颁发JWT Token，网关统一验证 Token，有效期/权限解析在 JWT 中完成。
- CAS 单点登录： CAS 作为统一登录入口，所有子系统重定向到 CAS Server 认证。登录成功后颁发 Ticket，子系统校验后建立本地会话

当前主流： OAuth2/OIDC 或 JWT + 网关 的组合，既标准又灵活。


[分布式认证鉴权最佳实践](https://mp.weixin.qq.com/s/N0E1QOsv-S1LGcqIrWHCZQ)
架构方案： 在分布式/微服务架构中，登录认证通常由一个 独立的登录认证服务集中处理，避免各服务重复实现登录逻辑。
1. 网关层鉴权:网关鉴权是将所有认证鉴权逻辑集中在API网关层处理，下游微服务无需关心认证鉴权问题，但是需要将各个微服务的权限信息都统一配置在网关中。
    - 优点：网关层集中处理，简化微服务逻辑。适合小型团队快速开发迭代、对安全的要求不高的项目中使用。
2. 各微服务自行鉴权：主要是把Token验签和权限识别都放在各个微服务之中，网关只做网络转发。
3. 网关+微服务的混合式鉴权。
   - 处理流程：网关只提供验签服务，对前端发起请求中的 Token 进行有效性校验，并提取Token中的用户和权限信息，将提取到信息放到Header中进行转发，同时标记当前请求已被网关处理。
     这种方式可以有效的避免分散式鉴权中各个微服务重复解析Token 而带来的时间成本，也能减少集中式鉴权中权限信息维护在网关的配置复杂度。
     各个微服务在接收到网关转发的请求后，直接读取Header中的用户权限信息，构建 Authentication 对象，后续使用 SpringSecurity 进行权限验证。

统一认证服务模块设计：

```text
├── commons/          # 通用基础模块
│   └── security      # 安全认证，包括基础的安全配置、异常定义、权限识别等
├── auth/             # 认证中心
│   └── user          # 用户服务（登录）
├── infra/            # 基础设施
│   └── gateway       # 网关服务，提供JWT验签、动态路由等。
└── business/         # 业务服务
    └── order         # 订单服务
```
...
除此之外，重要的还有用户身份管理(IDM)、身份认证(AM)、单点登录(SSO)、RBAC权限管理等。高性能微服务JWT + Gateway政企内网 / 传统系统CAS 单点登录
...
### 5.4.SSO方案1-CAS
SSO：本质是“统一认证入口+信任共享”，解决“多系统登录繁琐”和“认证逻辑冗余”的问题
1. CAS。统一认证中心
2. JWT。携带认证信息

关键概念：
1. TGT（Ticket Granting Ticket）全局会话凭证，存在认证中心的Cookie里，证明用户“已经登录过认证中心”； 
2. ST（Service Ticket）临时服务票据，一次性、有有效期，业务系统用它向认证中心“验明正身”。

标准流程：
1. 首次登录系统A用户访问系统A，系统A发现未登录，重定向到SSO认证中心，附带回调地址：sso.com/login?redirect=systemA.com；
2. 用户在认证中心输入账号密码，验证通过；
3. 认证中心生成TGT（全局会话），写入自己域名下的Cookie（仅sso.com可访问）；
4. 认证中心生成ST（临时票据），重定向回系统A：systemA.com?ticket=ST-123；
5. 系统A拿着ST，通过后端接口调用认证中心验证；
6. 验证通过，认证中心返回用户信息，系统A创建自己的本地Session，用户成功访问系统A。

免登访问系统B
1. 用户访问系统B，系统B发现未登录，重定向到SSO认证中心；
2. 认证中心检测到Cookie里有有效的TGT（证明已登录），直接生成新的ST，跳过登录页面，重定向回系统B；
3. 系统B拿着ST去认证中心验证，通过后创建本地Session，用户免登访问系统B。

<p style="color: red">认证中心直接把用户ID、用户名放在重定向URL里，系统A不就直接能用了？何必搞ST这么复杂？</p>

答案是安全！URL是明文传输，会被浏览器历史记录、服务器日志留存，用户信息直接暴露，风险极高。
而ST的优势在于：一次性有效：验证完就失效，就算被截获也没用；时效性短：通常几分钟就过期，窗口风险极小；
后端验证：用户信息只在认证中心和业务系统的后端交互，前端完全接触不到。

<p style="color: red">CAS的坑：生产环境要注意什么？</p>

CAS虽经典，但在分布式、高并发场景下有明显短板：
1. 网络开销大每个业务系统验证都要调用认证中心，高峰期容易成为瓶颈；
2. 单点风险SSO认证中心挂了，所有系统都无法登录；
3. 跨域Cookie风险TGT存在Cookie里，跨域场景下可能遭遇CSRF攻击。
...
实现流程：
1. 用户在SSO认证中心登录，认证中心生成JWT，返回给前端；
2. 前端存储JWT（存储位置后面详细说）；
3. 用户访问系统A，前端在HTTP请求头的Authorization字段中带上JWT（格式：Bearer <token>）；
4. 系统A用服务端密钥验证JWT签名：签名通过则解析Payload获取用户信息，直接完成认证，无需调用认证中心。

- 优点：各业务系统独立验证Token，不用依赖认证中心，性能高、扩展性好，完美适配微服务。
- 缺点：
  1. JWT本身不存储Session，无法实现“记住我”功能，也就无法主动失效。解决方案：后端存储到Redis中，配合定时任务清理失效的Token。    
  2. JWT的存储位置需要前端保存，不能跨域，需要单独实现跨域传输的机制。

<p style="color: red">JWT该存在哪里？</p>
敏感系统用“HttpOnly Cookie + SameSite=Strict”（防XSS和CSRF）；非敏感系统用LocalStorage，前端自行管理。

### 5.6.SSO方案3-OAuth
上述方案都存在 无法退出（A系统退出，其他系统也必须退出）、跨域（尤其是同主域名不同子域名间的共享认证）

**注意**：OAuth2.0是“授权”，SSO是“认证”。可以用OAuth2.0实现SSO，但不能说两者是一回事！
...
利用 User-Agent、HTTP请求头信息、屏幕分辨率和颜色深度、时区和语言设置、浏览器插件详情等计算生成

现成的JavaScript库，像 FingerprintJS 或 ClientJS

FingerprintJS。
- 官网[https://fingerprint.com/](https://fingerprint.com/)
- 开源：[https://github.com/fingerprintjs/fingerprintjs](https://github.com/fingerprintjs/fingerprintjs)


ClientJS。主要功能：浏览器指纹、前端客户端的各种数据（和指纹相关的）
- 源码[https://github.com/jackspirou/clientjs](https://github.com/jackspirou/clientjs)
