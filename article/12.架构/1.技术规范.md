...
OpenAPI标准总结：设计一种标准的、与语言无关的http api接口。
1. 接口设计。选择协议，接口风格。以restfull 接口为例。
   1. 接口风格为 资源导向（使用名称而非动词）
   2. http方法语义（GET/PUT/POST/DELETE）
   3. 统一响应格式，例如使用JSON格式，包括state、msg、data、requestId、error_code等字段信息。
   4. 状态码(非http状态码，而是请求的状态码)，包括定义状态码的错误信息。
2. 版本控制。通常使用路径添加版本号，例如： /api/v1/user
3. 接口文档。开发/测试环境可以暴露出swagger标准的接口文档，提供清晰、明了的接口说明。生产环境关闭。
4. 功能性设计。
   1. 限制请求的数据量。例如分页（限制深度分页查询、限制最大单页条数），请求参数数量（每次提交数量最多100条等）
   2. 幂等性设计：一个请求重复请求，产生同样的影响。例如前端按钮限制，后端分配token、乐观锁等。
   3. 性能优化。缓存（Redis/Memcached）、异步处理（消息队列）、分页 / 批量查询、熔断限流（Hystrix/Sentinel）。做好压力测试。
   4. 设计合理的超时时间和重试次数。
   5. 设计合理的服务降低与熔断机制。避免被上游接口拖死。
      - 超时降级（调用服务时超时返回默认值或者其它处理办法）
      - 失败次数降级（多次调用失败后，服务可用率下降，自动摘除异常节点或者返回默认值等）
      - 限流：也是一种降级
      - 故障降级，也就是熔断机制，主要体现在调用放。例如外部监控系统发生故障进行服务熔断、代理服务（nginx）检测到下游全部异常而返回默认值等。
5. 安全性。
    1. 身份认证和授权。
       - 常见方案：API KEY、jwt、oauth2等。防止未授权用户获取数据；
       - 遵循原则：最小权限原则、敏感信息脱敏/加密等。
       - 重点：防窃取（https）、防篡改(https+签名)、防重放(过期时间)。
    2. 安全机制。使用https、防止sql注入、xss、流量限速（防止暴力破解）、认证有效期（防止请求重放攻击）。
...
主流的版本号标准有两个
1. Semver (Semantic Versioning) 语义化版本号， 官网[https://semver.org/lang/zh-CN/](https://semver.org/lang/zh-CN/)
2. CalVer (Calendar Versioning) 日历化版本好，官网[https://calver.org/overview_zhcn.html](https://calver.org/overview_zhcn.html)
