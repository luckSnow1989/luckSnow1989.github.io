...
批处理是将一个大任务拆分成多个小任务，然后并行执行，最后将结果合并，常见的批处理框架有：
1. 中小规模 Java 批处理：优先选 JobRunr（轻量）或 Quartz（调度为主），替代 Spring Batch 降低复杂度； 
2. 大数据量级批处理：选 Apache Spark（内存计算）或 Flink Batch（流批一体），适配海量数据场景； 
3. 数据流水线 / 多步骤批处理：选 Airflow/Prefect（Python 生态）或 Temporal（跨语言），支持复杂任务依赖； 
4. 可视化 / 低代码批处理：选 Apache NiFi，适合非开发人员或快速搭建批处理流程； 
5. 跨语言 / 长任务批处理：选 Temporal，兼顾容错性和多技术栈兼容。

核心原则：小批量、轻量需求→JobRunr/Quartz；大数据、高性能→Spark/Flink；复杂流程、可视化→Airflow/NiFi。

Spring Batch 是一个用于批量处理数据的框架，它支持大规模数据的批量导入、处理和导出，可以与数据库、文件和消息队列集成，适用于数据迁移、数据分析、报表生成等场景。

Spring Batch 的核心概念包括：
1. Job：批处理任务的顶层容器，包含多个 Step。
2. Step：Job 的基本执行单元，包括读取（Reader）、处理（Processor）和写入（Writer）。
3. ItemReader：从数据源读取数据。
4. ItemProcessor：处理数据（如转换、过滤）。
5. ItemWriter：将处理后的数据写入目标存储。

Spring Batch工作流程：Job → Step → Reader/Processor/Writer。
```text
Job
