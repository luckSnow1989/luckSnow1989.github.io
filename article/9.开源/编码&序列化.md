...
## Apache Arrow Flight
Arrow Flight 是基于 Arrow 的 RPC 通信协议，其利用 Arrow 的标准化内存表示，使得数据在不同服务之间传输时无需序列化/反序列化，极大降低了CPU开销和延迟。
尤其在 Java 生态中，通过 arrow-flight-java 客户端可直接读取远程数据并集成至 Spark、Flink 等计算引擎。

[Apache Arrow Flight 官方文档](https://arrow.apache.org/docs/java/flight.html)

核心库
- Arrow Memory：零拷贝列式内存结构（Arrow）
- Arrow Flight：高速 RPC 数据传输协议（gRPC + Arrow）
            ...
            try (FlightStream stream = client.getStream(ticket)) {
                ...
                while (stream.next()) {
                    for (int i = 0; i < root.getRowCount(); i++) {
                        int id = root.getVector("id").getObject(i);
...
3. 启动 Spring Boot 应用后，控制台输出：
🚀 Arrow Flight Server started on grpc+tcp://0.0.0.0:8815

4. 然后运行客户端：🚀 Connecting to Arrow Flight server...
Row 0: id=1, name=Alice
Row 1: id=2, name=Bob
✅ Finished reading Arrow Flight stream.
```