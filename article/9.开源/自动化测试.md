...
当你想进行单元测试的时候，立刻会遇到几个致命的问题。
1. 依赖外部环境。例如需要连接数据库，如果数据库没有启动或、网络不同、配置错误的时候，测试就会失败，但这并非是程序的错误。
2. 测试速度慢:涉及数据库或网络 I/O的操作常缓慢。成百上千个这样的测试会让整个构建过程变得无法忍受。
3. 难以控制场景:如何测试“用户名已存在”的场景?你必须先在数据库里插入一个同名用户。如何测试“数据库保存失败的场景?这几乎无法在测试中稳定地模拟。
4. 测试不独立:一个测试在数据库里插入了数据可能会影响到下一个测试的结果，这破坏了测试的独立性。


Mockito 的解决方案:以 UserService 调用 UserRepository 为例

Mockito 通过创建 模拟对象(Mock Object) 来斩断这些依赖，从而完美地解决了上述所有问题
1. 隔离(lsolation):Mockito 会创建一个 UserRepository 的“假”对象(替身)。这个假对象完全不连接数据库。你的UserService和这个假对象交互，从而与数据库完全隔离。
2. 速度(Speed):所有操作都在内存中完成，速度极快。
3. 控制(Control):你可以像导演一样为这个假”对象编写剧本。
   - 想测试“用户名已存在”? 	when(userRepository.findByUsername("existingUser")).thenReturn(newUser());
   - 想测试“用户不存在”?   when(userRepository.findByUsername("newUser")).thenReturn(null);
   - 想测试“数据库保存失败”? ption(when(userRepository.save(any(User.class))).thenThrow(new DataAccessExcer.class("....")));
4. 验证（verification）：不仅可以控制方法的返回值，还能在测试后验证UserService是否按照预期完成与UserRepository的交互。

实现原理：在运行时创建代理对象，字节码增强使用的事byte Buddy实现的。

#### 3.1.1.【核心原则-隔离】解决测试类对其他类的依赖
