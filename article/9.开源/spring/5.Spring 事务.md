...
- NESTED：如果当前存在事务，则创建一个子事务作为当前事务的嵌套事务来运行，最终子事务和父事务一起提交；如果当前没有事务，则该取值等价于REQUIRED。
...
public void save3() {
  // 1. 数据库操作
  // doSomething
  // 2. 判断是否开启了事务。  将操作注册到“afterCommit”阶段
  // 在事务提交之后，立即执行注册的方法。且这个方法时无法开启事务的
  if (TransactionSynchronizationManager.isSynchronizationActive()) {
      TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronizationAdapter() {
          @Override
          public void afterCommit() {
              // 做些跟事务没有关系的事情 比如。发送mq
          }
      });
  }
}
