...
我们已经把需要代理对象的功能，组合成一个合集（List<MethodInterceptor>），以 责任链模式 依次调用，调用链如下：
...
> 在aop方面，关于性能，我们其实可以得出一个排序：AspectJ > JDK > CGLib
...
<p style="color:red">问题：springboot中的@Aspect 和 AspectJ是一回事吗？</p>

Spring Boot 中最常用的 AOP 方式（通过 @Aspect 定义切面、@Before/@After 等注解定义通知），是 Spring 框架对 AspectJ 注解的 “借用”。
但底层实现依赖的是 Spring 自身的 动态代理机制，而非 AspectJ 框架的字节码增强。
因为：spring需要更加灵活的扩展性，以及热更新等特性。字节码增强是无法满足需求。