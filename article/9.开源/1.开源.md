...
- [排名前 50 的 Apache 项目](https://www.softpost.org/tech/top-50-apache-projects)
...
- 飞致云。1panel、jumpserver、dataease、cordys CRM、maxkb、halo等。

## 主流开源框架

1. 数据库相关：涉及连接、数据通信（持久化）、数据分片、事务、连接管理等。
    1. 主流数据库
       ...
       - 分布式数据库：如TiDB，OceanBase等，都是兼容MySQL的分布式数据库。
    2. 数据库连接池
   ...
   4. 数据分片：实现的分案很多：Cobar， MyCAT， TDDL， DRDS， Atlas， DBProxy， sharding-JDBC， Heisenberg， CDS， DDB， OneProxy， Oceanus， Vitess， Kingshard， MaxScale与MySQL Route
      - sharding-sphere。分库分表的插件，包括Sharding-JDBC、Sharding-Proxy、Sharding-Sidecar 捐献给Apache，目前由京东维护
      - MyCAT：数据库代理，可以实现分库分表
   5. 分布式ID 数据分片后，最先要解决的就是ID的问题。 Snowflake是推特的一个完美解决方案，目前的实现方案如下：
   ...
   6. 分布式事务：一般事务的管理，都是orm框架封装实现，由spring进行隐式管理。这里只讲分布式事务。
      - Atomikos：基于jta和数据库XA驱动的分布式事务插件，也就是在一个项目中调用多个数据源时可以使用的框架，实现的原理就是2pc
      - Seata：阿里实现的分布式事务框架。
      - TCC分布式事务：ByteTCC、himly、tcc-transaction
2. 综合性框架
   - spring：  一个开放源代码的设计层面框架，它解决的是业务逻辑层和其他各层的松耦合问题，包括mvc、springboot、springcloud等。
   - Jfinal 国产orm+web框架，在国内有一定的用户量。
   - Quarkus。RedHat 推出的 “容器优先” 框架，专为 Kubernetes 优化，支持 AOT 编译和 JIT 模式，启动快、内存占用低。
   - Solon。由杭州无耳科技有限公司（下属 Noear 团队）开发并开源。是新一代，Java 企业级应用开发框架。
3. 页面与控制层框架
...
11. 数据编码&解码（序列化）
    ...
    - Tika。apache顶级项目，数据类解析。PDF、Word、Excel、PPT，甚至是 HTML、XML、TXT、RTF，它都能帮你提取文本和元数据信息（作者、标题、创建时间之类的）。
12. 网络通信
...
13. 定时任务
    ...
    - saturn 唯评会开源的任务调度框架
14. 基础工具类框架
     1. Apache Commons：包含了很多开源的工具，用于解决平时编程经常会遇到的问题，减少重复劳动
     ...
     3. 参数校验
        - 实现JSR 380的方案：Hibernate Validator（最早、最多使用的），Apache BVal（单独实现，非常轻量）
        - 独立实现方案：Spring Validator（内置工具非注解、编程式）
        - 函数式：Vavr Validation(函数式编程风格，支持“错误累积”（不像异常只返回一个错误）)、Fluent Validator（链式调用）
     4. 对象转换
...
- 京东：[https://gitee.com/jd-platform-opensource](https://gitee.com/jd-platform-opensource)
  - asyncTool。并发编排工具。
  - hotkey。热数据探测与通知。使用教程：https://juejin.cn/post/7427778132609024041
...
- rustdesk。远程桌面，开源的向日葵工具。[https://github.com/rustdesk/rustdesk](https://github.com/rustdesk/rustdesk)
- firecrawl。将页面转为markdown。[https://github.com/firecrawl/firecrawl](https://github.com/firecrawl/firecrawl)
- DWSurvey。开源问卷：https://github.com/wkeyuan/DWSurvey
...
- Silky：构建于Spring Boot之上的组件生态，并非脚手架那么简单。[https://github.com/yijuanmao/silky-starter](https://github.com/yijuanmao/silky-starter)
...
- Snowy。Snowy（SnowyAdmin）是国内首个国密前后端分离快速开发平台。[https://github.com/xiaonuobase/Snowy](https://github.com/xiaonuobase/Snowy)

### ERP

- 管伊佳ERP（曾用名：华夏ERP，英文名：jshERP）是一款在国内广受欢迎的开源免费ERP系统。它深度整合了进销存、财务、生产等核心业务流程，为中小企业提供了一套完整、高效的管理解决方案。
  [https://gitee.com/jishenghua/JSH_ERP](https://gitee.com/jishenghua/JSH_ERP)

### 商城
当前的商城都是单商户版本的，适合小个体商使用。
- CRMEB。CRMEB 开源商城系统Java版基于Java+Vue+Uni-app开发，在微信公众号、小程序、H5移动端都能使用，代码全开源无加密，
   独立部署，二开很方便，还支持免费商用，能满足企业新零售、分销推广、拼团、砍价、秒杀等多种经营需求，自用、做二开项目都很合适。
   [https://gitee.com/ZhongBangKeJi/crmeb_java](https://gitee.com/ZhongBangKeJi/crmeb_java)
...
### 商用
汇聚了很多可以直接商用的项目(大部分来自开源收集、培训学院等)[https://gitee.com/itxinfei/projects](https://gitee.com/itxinfei/projects)
- 云岚到家 jzo2o。是一个家政 O2O 平台，包含用户端、机构端和服务人员端，支持家政服务的在线交易流程。[https://gitee.com/itxinfei/jzo2o](https://gitee.com/itxinfei/jzo2o)
- 中州养老院智慧管理系统。基于SpringBoot的智慧养老综合解决方案，整合入住管理、服务记录、财务管理、家属交互等核心业务模块，助力养老机构数字化转型。[https://gitee.com/itxinfei/zzyl](https://gitee.com/itxinfei/zzyl)
- 餐掌柜 RestKeeper 智能餐饮管理系统。由传智播客研究院研发的智慧餐饮 SaaS 服务及运营平台，致力于为餐饮集团或门店提供全套互联网化、智能化的收银与管理解决方案。[https://gitee.com/itxinfei/restkeeper](https://gitee.com/itxinfei/restkeeper)
- 乐尚代驾。基于 Spring Cloud 微服务架构的智能化代驾服务平台，支持酒后代驾、商务代驾、长途代驾等核心业务场景。[https://gitee.com/itxinfei/daijia-parent](https://gitee.com/itxinfei/daijia-parent)
- 学成在线。一个基于互联网+的 IT 职业教育平台，提供课程学习、练习、考试等完整教学服务。[https://gitee.com/itxinfei/xuecheng-parent](https://gitee.com/itxinfei/xuecheng-parent)
...
- 陀螺匠。是一套采用Laravel 9框架结合Swoole高性能框架与Vue.js前端技术栈构建的AI+低代码双引擎智能办公系统。
  [https://gitee.com/ZhongBangKeJi/tuoluojiang](https://gitee.com/ZhongBangKeJi/tuoluojiang)
...
- qiwen-file。网盘。[https://gitee.com/qiwen-cloud/qiwen-file](https://gitee.com/qiwen-cloud/qiwen-file)

### 低代码

- smardaten无代码平台，有社区版，没有源码。[https://s3.smardaten.com](https://s3.smardaten.com)
- halo。强大易用的开源建站工具。[https://www.halo.run/](https://www.halo.run/)
...
- 雷池-SafeLine[https://waf-ce.chaitin.cn/](https://waf-ce.chaitin.cn/)。开源地址:[https://github.com/chaitin/SafeLine](https://github.com/chaitin/SafeLine)
- FastMonitor。网络流量监控与威胁检测。[https://github.com/vam876/FastMonitor/](https://github.com/vam876/FastMonitor/)

### 金融

1. 财务
   - Invoify:10分钟部署、秒发邮件、支持多币种，独立开发者必备发票系统！[http://github.com/al1abb/invoify](http://github.com/al1abb/invoify)
2. 支付
    - IJPay 让支付触手可及，封装了微信支付、QQ支付、支付宝支付、京东支付、银联支付、PayPal支付等常用的支付方式以及各种常用的接口。
      不依赖任何第三方 mvc 框架，仅仅作为工具使用简单快速完成支付模块的开发，可轻松嵌入到任何系统里。[https://github.com/Javen205/IJPay](https://github.com/Javen205/IJPay)
...
1. 一款专门揭露屎山代码的质量分析工具。[https://github.com/Done-0/fuck-u-code](https://github.com/Done-0/fuck-u-code)
2. log-lottery。基于 Express + Three.js的 3D 球体抽奖程序。[https://github.com/LOG1997/log-lottery](https://github.com/LOG1997/log-lottery)

### 数据

- ERD Online。是全球首个开源且免费的在线数据建模与元数据管理平台，提供易用的元数据设计、关系图设计、SQL 查询等能力，
   并配备版本管理、导入导出、数据源管理、SQL 解析、审计与团队协作等功能，帮助我们快速、安全地管理数据库元数据。
...
- BigDataView。目前整理了118套大数据可视化模板，涵盖政务、交通、医疗、教育、公安、环保、旅游、农业、工业、物流、物联网等各个行业，满足不同用户的需求。而且代码使用简单，可快速搭建集成到自己的大屏展示页面。
   [https://gitee.com/iGaoWei/big-data-view](https://gitee.com/iGaoWei/big-data-view)

### 信息获取&发布

- TrendRadar。简单的舆情监控分析 - 多平台热点聚合+基于 MCP 的AI分析工具。监控35个平台（抖音、知乎、B站、华尔街见闻、财联社等），
  智能筛选+自动推送+AI对话分析（用自然语言深度挖掘新闻：趋势追踪、情感分析、相似检索等13种工具）。支持企业微信/飞书/钉钉/Telegram/邮件/ntfy推送，
  30秒网页部署，1分钟手机通知，无需编程。支持Docker部署，让算法为你服务，用AI理解热点。[https://github.com/luckSnow1989/TrendRadar](https://github.com/luckSnow1989/TrendRadar)
- PinMe。利用 IPFS（分布式存储网络-区块链技术的基础能力）来存储文件，并自动生成一个免费的、基于 ENS 域名的永久链接。可以一键将静态网站快速部署到网上，搞出来一个链接，微信发出去，别人就能访问了。
  - 开源地址：[https://github.com/glitternetwork/pinme](https://github.com/glitternetwork/pinme)。
  - 也可以直接使用部署好的服务：[https://pinme.eth.limo/](https://pinme.eth.limo/)

## 全栈开发

1. Prisma ORM 是一款专为 Node.js 和 TypeScript 设计的现代化数据库工具链，提供类型安全、自动化迁移、可视化数据管理等功能，适用于构建高效的数据库驱动应用。
  [https://github.com/prisma/prisma](https://github.com/prisma/prisma)
2. firebase。Firebase 是 Google 提供的一站式后端云服务平台，支持 Web、iOS、Android 等多端开发。
   它集成了实时数据库、用户认证、云存储、消息推送、远程配置等众多功能，让开发者无需搭建服务器，即可快速开发和上线高质量的应用。
3. supabase[https://github.com/supabase/supabase](https://github.com/supabase/supabase)
