...
### 8.1.网络边界防护类-防火墙/NGFW

原理：基于策略的访问控制，通过状态检测技术跟踪会话状态，NGFW增加L7应用识别能力
...
![](img/safe/82575cd1.png)


### 8.2.网络边界防护类-IPS/IDS

原理：基于特征匹配（Snort规则）或异常行为分析，IDS旁路检测，IPS串联阻断

功能与作用：
- 实时检测并阻断已知攻击（漏洞利用、蠕虫传播） 
- 识别异常网络行为（内部横向移动） 
- 满足合规性要求（等保2.0）
- 提供攻击取证数据

![](img/safe/54947a0b.png)


### 8.3.网络边界防护类-网闸

原理：物理切断TCP/IP链路，通过专用硬件进行应用层数据摆渡
...
原理：深度解析HTTP/HTTPS流量，通过规则引擎+机器学习阻断应用层攻击

功能与作用：
- 防护OWASP Top 10攻击（SQL注入/XSS等）;
- 防止敏感数据泄露（信用卡/身份证号）;
- 缓解CC攻击和应用层DDoS;
- 提供虚拟补丁（未修复漏洞临时防护）

![](img/safe/249c7efd.png)

### 8.5.应用与数据安全类-防篡改

原理： 文件驱动层监控，建立文件哈希基线，实时拦截未授权修改

功能与作用：
- 防止网站文件被恶意篡改;
- 实时恢复被破坏内容;保护网站完整性（政府/金融网站）;
- 与WAF联动封禁攻击源

![](img/safe/4565fa16.png)

### 8.6.应用与数据安全类-DLP数据泄露防护系统
...
原理："永不信任，始终验证"，基于身份/设备/环境的动态访问控制
...
![](img/safe/49d705de.png)


### 8.11.安全监控与分析类

1. 态势感知:多源数据采集+关联分析+ATT&CK攻击链建模，能够对整个网络环境进行实时监控和分析，及时发现和响应安全威胁。
2. 蜜罐:通过模拟攻击者行为，诱捕攻击者进入陷阱，从而收集攻击者的信息。
3. 全流量分析:深度包检测(DPI)+元数据分析+行为建模，对网络中的所有流量进行捕获、分析和监控，可以发现隐藏在正常数据流中的恶意活动。
4. 日志审计系统：对网络中的日志数据进行收集、分析和报告，可以发现异常行为和潜在的安全威胁。
5. 堡垒机：运维操作代理+会话录像。对运维操作进行审计和监控，防止误操作或恶意行为。

### 8.12.漏洞管理类

1. 漏洞扫描：主动探测+特征匹配+认证扫描。分多种，例如通过web扫描、服务扫描、代码扫描等。
...
- [国内外知名免费及商业漏洞库汇总](https://blog.csdn.net/u013129300/article/details/130165182)
