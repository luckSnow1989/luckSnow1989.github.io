...
[网络代理，也就是VPN](https://github.com/shadowsocks/shadowsocks/tree/master)

Netty是一个高性能、异步事件驱动的网络应用框架，用于快速开发可维护的高性能协议服务器和客户端。它极大地简化了TCP和UDP套接字服务器等网络编程，例如TCP和UDP套接字服务器。

#### 2.1.1.核心组件

Netty的核心组件包括：Channel、EventLoop、ChannelHandler、ChannelFuture、ChannelPipeline。

![](img/http/b421664e.png)

1. Channel：代表一个可以进行读写操作的通道，可以用于网络数据的读写、文件的读写等操作。
2. EventLoop：负责处理所有的事件，包括读事件、写事件、连接事件、断开连接事件等。
3. ChannelFuture：用于获取异步操作的结果，例如：写操作、连接操作等。
4. ChannelHandler：用于处理各种事件，包括读事件、写事件、连接事件、断开连接事件等。
5. ChannelPipeline：负责管理各种ChannelHandler，用于处理网络数据的读写、协议解析等操作。

#### 2.1.2.粘包拆包

- [Netty复现与解决方案](https://zhuanlan.zhihu.com/p/89602783)
- [Netty整理](https://my.oschina.net/u/3768341/blog/3105228)
...
Netty采用的是基于事件驱动的Reactor模型，用于处理多个I/O源的事件分发。就是“同步非阻塞”IO模型。

Reactor模型有三种线程模式(区别在于事件接受器与事件处理器的线程数量)：单线程(一对一)、多线程(一对多)、主从线程(多对多)。

实现原理：Reactor模式主要通过Selector和ChannelHandler来实现。
1. Selector用于监听多个Channel的事件(等待epoll的回调事件)，当某个Channel上的事件就绪时，Selector会将其放入就绪队列中。
2. 然后，Netty的事件循环（EventLoop）会轮询就绪队列中的事件，并调用相应的ChannelHandler进行处理。

- Channel表示程序到某个实体（如硬件设备、文件、网络套接字或者能够执行I/O操作的程序组件）的开放连接，如读操作和写操作。 
- ChannelHandler是Netty中处理I/O事件或拦截I/O操作的组件，它负责处理网络事件，如接收数据、写入数据等。开发者可以自定义ChannelHandler来实现自己的业务逻辑。
- ChannelPipeline是ChannelHandler的链表，采用责任链的模型处理Channel中的事件。当某个事件发生时会依次执行所有的ChannelHandler直到找到能够处理该事件的ChannelHandler。

### 2.1.4.Netty的零拷贝

在 OS 层面上的零拷贝通常指避免在 用户态(User-space) 与内核态(Kernel-space) 之间来回拷贝数据。
而在 Netty 层面 ，零拷贝主要体现在对于数据操作的优化。主要体现在以下几个方面：
1. 使用 Netty 提供的 CompositeByteBuf 类, 可以将多个 ByteBuf 合并为一个逻辑上的ByteBuf, 避免了各个 ByteBuf 之间的拷贝。
2. ByteBuf 支持 slice 操作, 因此可以将 ByteBuf 分解为多个共享同一个存储区域的ByteBuf, 避免了内存的拷贝。
...
Linux 常用的抓包工具，也用于进行流量回放，Linux系统一般都默认安装。只有命令行，图形化分析需要借助Wireshark展示。
