...
- 头部压缩。当前我们在head上做了非常多的处理，导致头部过重，经过压缩后最多可压缩90%以上，降低带宽。客户端和服务端都会有一个预定的61个静态表，服务端维护动态表记录不在静态表的被压缩的头信息。
- 二进制分帧。将数据拆分为更小的帧，提高数据解析效率
- 轻量化服务器推送功能。替代webSocket。
...
QUIC 协议如何解决 TCP 的缺陷？
1. 基于 UDP：绕过操作系统内核的 TCP 协议栈，避免队头阻塞。
2. 0-RTT 连接：通过缓存会话密钥实现快速重连。
3. 连接迁移：使用 Connection ID 而非 IP + 端口，支持网络切换不断连。
4. 前向纠错（FEC）：发送冗余数据包减少重传次数。
...
HTTPS双向认证（Mutual TLS，mTLS）是一种增强型安全协议，在一些安全级别较高的场景非常有用，要求客户端和服务器均验证对方的数字证书，确保双方身份可信

准备：
1. 根证书（CA Certificate）：由可信证书颁发机构（CA）签发，或自建私有CA。
2. 服务器证书：由CA签发，包含服务器域名/IP，私钥由服务器保管。
3. 客户端证书：由CA签发，包含客户端标识（如用户ID），私钥由客户端保管。

适用场景
1. 企业内网服务：API网关验证客户端设备合法性。 
2. IoT设备通信：确保设备与云端双向可信。 
3. 金融交易系统：支付接口要求客户端（商户）强身份认证。 
4. 零信任架构：替代传统VPN，基于证书的动态访问控制。
...
sequenceDiagram
    ...
    Server->>Client: ServerHello（选定加密套件）+ 服务器证书
    Note over Client: 验证服务器证书有效性
    Client->>Server: 客户端证书（需双向认证时）
    Note over Server: 验证客户端证书有效性
    Server->>Client: CertificateRequest（请求客户端证书）
    Client->>Server: CertificateVerify（签名验证）
    Server->>Client: ServerHelloDone
    Client->>Server: 生成会话密钥（Pre-Master Secret）
    Server->>Client: 确认密钥交换完成
